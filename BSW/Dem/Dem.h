 ...
 *  12.01.00  2016-08-29  vismhe  ESCAN00091568 Support OperationCycle automatic end
 *            2016-09-14  visanh  FEATC-14      FEAT-1900: S/R Communication for DEM
 *            2016-09-14  visanh  ESCAN00091574 UnconfirmedDTC is not reset
 *            2016-09-14  visanh  ESCAN00091868 Operation cycle counters not persisted in NvRAM
 *            2016-09-20  visanh  ESCAN00091953 Dem_Mem_EntrySetOcc1(MemoryEntry, 0x00U) is called twice in Dem_Data_EntryInit
 *            2016-09-30  vismhe  ESCAN00092086 Runtime check does not prevent out-of-bounds memory access
 *            2016-10-11  visanh  ESCAN00091966 Suppressed DTC is reported by service 19x03
 *            2016-10-11  visanh  ESCAN00091967 WIR status bit is reset for aged, but not healed DTC on ECU restart
 *            2016-10-11  visanh  ESCAN00091970 Pending status bit is reset too early on ECU restart
 *            2016-10-11  visanh  ESCAN00091971 Improve recovery for healing after power loss
 *            2016-10-11  visanh  ESCAN00092066 DTCs do not confirm
 *            2016-10-11  visanh  ESCAN00092085 OBD Freeze Frame erroneously cleared
 *            2016-10-14  vissat  ESCAN00091755 PID 0x01: OBD relevant montioring functions are always completed
 *            2016-10-14  vismhe  ESCAN00092317 Combined DTC completed status is incorrect
 *            2016-11-07  visanh  ESCAN00092330 Compiler warning: function "Dem_Esm_EventSetDisconnectedBit" was declared but never referenced
 *            2016-11-07  visanh  ESCAN00092328 Compiler warning: function "Dem_Mem_SetFdcTocStatus" was declared but never referenced
 *            2016-11-07  visanh  ESCAN00092329 Compiler warning: function "Dem_Mem_IndicatorSetEventCountBlinking" was declared but never referenced
 *********************************************************************************************************************/
...
#define DEM_SW_MAJOR_VERSION                     (12U)
...
#define DEM_NVMINIT_APIID                                 (0xf1U)              /*< Service ID: Dem_NvM_InitAdminData(), Dem_NvM_InitStatusData(), Dem_NvM_InitDebounceData(), Dem_NvM_InitEventAvailableData(), Dem_NvM_InitObdFreezeFrameData, Dem_NvM_InitObdPermanentData(), Dem_NvM_InitObdIumprData() */
 ...
 *                Especially Dem_ReportErrorStatus() is supported to allow
 ...
 *                Otherwise it behaves similar to Dem_SetEventStatus().
 *
 ...
 *
 *                Dem_GetEventStatus() is part of the RTE interface, but
 ...
 *
 * \details       This API writes TRUE to EventTested if the event has
 ...
 *                as Operation of port interface 'DiagnosticInfo'.
 *
 *                Dem_GetEventFreezeFrameData() copies data from the stored
 *                snapshot data of an event into DestBuffer, following no
 *                particular format.
 *
 *                With ReportTotalRecord == FALSE, only the DID DataId is
 *                copied, in case of ReportTotalRecord == TRUE the data of all
 *                DIDs is concatenated. In case of DIDs only the data actually
 *                stored in the event storage is returned here, if the Dcm adds
 *                additional parts to a DID, these will be missing.
 *
 ...
 *                as Operation of port interface 'DiagnosticInfo'.
 *
 *                Dem_GetEventExtendedDataRecord() copies extended data
 ...
 * Dem_Cfg_StatusData.EventStatus
 * Dem_Cfg_ObdIumprData.Status
 * Dem_Mem_PrestorageHeadIndex
 * Dem_Cfg_Prestorage.EventId
 * Dem_Cfg_Prestorage.State
 * Dem_Cfg_IndicatorContinuous
 * Dem_Cfg_IndicatorBlinking
 * Dem_Cfg_IndicatorFastFlash
 * Dem_Cfg_IndicatorSlowFlash
 * Dem_Cfg_GlobalSpecialIndicatorStates
 ...
 * Dem_NextEnableConditionStates
 * Dem_CurrentEnableConditionStates
 * Dem_StorageConditionStates
 * Dem_Queue_CycleFlag
 * Dem_Queue_CycleState
 *
 * \usedin
 * Dem_MainFunction
 * Dem_SetOperationCycleState
