...
#if ((IPDUM_SW_MAJOR_VERSION != 6U) || (IPDUM_SW_MINOR_VERSION != 5U) || (IPDUM_SW_PATCH_VERSION != 0U))
...
# if(IPDUM_TXSTATICPDUREFOFTXPATHWAY == STD_ON)
  if((IpduM_IsTxPartStaticPartUsedOfTxPathway(txPathwayId)) && (IpduM_IsJitUpdateOfTxPart(IpduM_GetTxPartStaticPartIdxOfTxPathway(txPathwayId))))
  {
    ...
    {
      retVal = IpduM_JitTriggerTransmit(IpduM_GetUlPduRefOfTxPart(IpduM_GetTxPartStaticPartIdxOfTxPathway(txPathwayId)), txPathwayId, IpduM_GetTxPartStaticPartIdxOfTxPathway(txPathwayId), pduInfoPtr);        /* SBSW_JITUPDATE_CALL_JITTRIGGERTRANSMIT */
    }
  }
# endif
  {
...
# endif
      {
        retVal = IpduM_JitTriggerTransmit(IpduM_GetUlPduRefOfTxPart(txDynamicPartId), txPathwayId, txDynamicPartId, pduInfoPtr);        /* SBSW_JITUPDATE_CALL_JITTRIGGERTRANSMIT */
      }
    }
  }
# if(IPDUM_TXSTATICPDUREFOFTXPATHWAY != STD_ON)
...
# endif
  {
    ...
    SchM_Exit_IpduM_IPDUM_EXCLUSIVE_AREA_0();
    if((containerWriteCount - containerReadCount) >= 1u)
    {
...
LOCAL_INLINE FUNC(void, IPDUM_CODE) IpduM_QueueCurrContainerPdu(IpduM_TxContainerBufferInfoIterType txContainerId)
{
  ...
  if((containerWriteCount - containerReadCount) >= bufferInstanceCount)
  {
...
LOCAL_INLINE FUNC(void, IPDUM_CODE) IpduM_TransmitOrQueueCurrContainerPdu(IpduM_TxContainerBufferInfoIterType txContainerId)
{
  Std_ReturnType transmitResult = E_NOT_OK;
# if (IPDUM_TRIGGERTRANSMITOFTXCONTAINERPDU == STD_ON)
  /* If direct transmission is configured: try this first. */
  if(!IpduM_IsTriggerTransmitOfTxContainerPdu(txContainerId))
# endif
  {
...
LOCAL_INLINE FUNC(void, IPDUM_CODE) IpduM_AddTxPduToCurrContainer(IpduM_TxContainedPduIterType txContainedId, P2CONST(PduInfoType, AUTOMATIC, IPDUM_APPL_DATA) PduInfoPtr)
{
  ...
  SchM_Enter_IpduM_IPDUM_EXCLUSIVE_AREA_0();
  {
    ...
    VStdMemCpyRamToRam(&pContainerTxBuf[headerSize], PduInfoPtr->SduDataPtr, PduInfoPtr->SduLength);    /* SBSW_ADDTXPDUTOCURRCONTAINER_CALL_MEMCPY */
    IpduM_SetContainedPduWriteIdxOfTxContainerBufferIdxInfo(txContainerId, IpduM_GetContainedPduWriteIdxOfTxContainerBufferIdxInfo(txContainerId) + headerSize + PduInfoPtr->SduLength);        /* SBSW_ADDTXPDUTOCURRCONTAINER_INDIRECT_ACCESS */
  }
  SchM_Exit_IpduM_IPDUM_EXCLUSIVE_AREA_0();
}
#endif
...
LOCAL_INLINE FUNC(void, IPDUM_CODE) IpduM_UnpackRxContainerPdu(IpduM_RxContainerPduIterType rxContainerId, P2CONST(PduInfoType, AUTOMATIC, IPDUM_APPL_DATA) PduInfoPtr)
{
  ...
  while(containedPduIdx < PduInfoPtr->SduLength)
  {
    IpduM_ContainedPduHeaderType header;        /* header.headerId; header.dlc */

    /* get header id and dlc of the contained PDU */
    header = IpduM_ContainerReadHeader(&(PduInfoPtr->SduDataPtr[containedPduIdx]), headerSize); /* SBSW_ANY_CALL_CONTAINERREADHEADER */
    ...
    if(((containedPduIdx) + header.dlc) <= PduInfoPtr->SduLength)
    {
...
# endif
        )
      {
        if(rxContainedPduId < IpduM_GetSizeOfRxContainedPdu())
        {
          ...
          PduR_IpduMRxIndication(IpduM_GetContainedPduRefOfRxContainedPdu(rxContainedPduId), &pduInfo); /* SBSW_UNPACKRXCONTAINERPDU_CALL_PDURRXINDICATION */
        }
      }
    }
...
LOCAL_INLINE FUNC(uint32, IPDUM_CODE) IpduM_searchRxHeaderId(uint32 headerId)
{
  ...
  for(rxContainedPduId = 0u; rxContainedPduId < IpduM_GetSizeOfRxContainedPdu(); rxContainedPduId++)
  {
...
LOCAL_INLINE FUNC(uint32, IPDUM_CODE) IpduM_CalculateSizeOfContainer(IpduM_TxContainerPduIterType txContainerId, P2CONST(PduInfoType, AUTOMATIC, IPDUM_APPL_DATA) PduInfoPtr)
{
  ...
  while(containerSize <= (PduInfoPtr->SduLength - headerSize)) /* stop iterating one headersize before the end to avoid reading a partial header */
  {
    ...
    currentHeader = IpduM_ContainerReadHeader(&(PduInfoPtr->SduDataPtr[containerSize]), headerSize);    /* SBSW_ANY_CALL_CONTAINERREADHEADER */
...
FUNC(void, IPDUM_CODE) IpduM_MainFunctionRx(void)
{
  ...
  if(IpduM_IsInitialized())
  {
    ...
    for(rxContainerId = 0u; rxContainerId < IpduM_GetSizeOfRxContainerBufferInfo(); rxContainerId++)
    {
      ...
      while((IpduM_GetContainerWriteCountOfRxContainerBufferIdxInfo(rxContainerId) - IpduM_GetContainerReadCountOfRxContainerBufferIdxInfo(rxContainerId)) > 0u)
      {
        pduInfo.SduDataPtr =
...
#  if (IPDUM_EXISTS_MUX_PATHWAYTYPEOFTXLOINFO == STD_ON)
      ...
      {
        retVal = IpduM_MuxTriggerTransmit(IpduM_GetTxPathwayIdxOfTxLoInfo(TxPduId), PduInfoPtr);        /* SBSW_TRIGGERTRANSMIT_CALL_MUXTRIGGERTRANSMIT */
        break;
      }
#  endif
#  if (IPDUM_EXISTS_CONTAINER_PATHWAYTYPEOFTXLOINFO == STD_ON)
      ...
      {
        retVal = IpduM_ContainerTriggerTransmit(IpduM_GetTxContainerPduIdxOfTxLoInfo(TxPduId), PduInfoPtr);     /* SBSW_TRIGGERTRANSMIT_CALL_CONTAINERTRIGGERTRANSMIT */
        break;
      }
#  endif
      default: /* COV_IPDUM_MISRA */ /* PRQA S 2018 */ /* MD_CSL_2018 */ 
...
#  endif
      SchM_Enter_IpduM_IPDUM_EXCLUSIVE_AREA_0();
      {
        ...
        VStdMemCpyRamToRam(PduInfoPtr->SduDataPtr, pMuxPduBuf, IpduM_GetLengthOfTxPathway(txPathwayHnd));       /* SBSW_MUXTRIGGERTRANSMIT_CALL_MEMCPY */
        PduInfoPtr->SduLength = IpduM_GetLengthOfTxPathway(txPathwayHnd);       /* SBSW_MUXTRIGGERTRANSMIT_PDUINFO_WRITE */
      }
      SchM_Exit_IpduM_IPDUM_EXCLUSIVE_AREA_0();
      retVal = E_OK;
#  if(IPDUM_JITUPDATEOFTXPART == STD_ON && IPDUM_UL_TRIGGER_TRANSMIT_API == STD_ON) /* COV_IPDUM_VAR_DEPENDENT_SWITCH */
...
# if (IPDUM_LASTISBESTOFTXCONTAINEDPDU == STD_ON)
    ...
    if(IpduM_IsLastIsBestOfTxContainedPdu(txContainedId))
    {
      ...
      {
        /* get header id and dlc of the contained PDU */
        header = IpduM_ContainerReadHeader(&pContainerTxBuf[containedPduBufferIdx], headerSize);        /* SBSW_ANY_CALL_CONTAINERREADHEADER */

        /* if header id and dlc match, replace data and break loop */
        if(header.headerId == IpduM_GetHeaderIdOfTxContainedPdu(txContainedId))
        {
          pduIsInContainer = TRUE;
          if(header.dlc == pduLength)
          {
            /* replace data in the buffer */
            VStdMemCpyRamToRam(&pContainerTxBuf[(containedPduBufferIdx + headerSize)], PduInfoPtr->SduDataPtr, PduInfoPtr->SduLength);  /* SBSW_CONTAINERTRANSMIT_CALL_MEMCPY */
            retVal = E_OK;
          }
          break;
        }
...
# if(IPDUM_CONTAINERTXBUFFER==STD_ON) /* COV_IPDUM_VAR_DEPENDENT_SWITCH */
      ...
      IpduM_AddTxPduToCurrContainer(txContainedId, PduInfoPtr); /* SBSW_CONTAINERTRANSMIT_CALL_ADDTXPDUTOCURRCONTAINER */
#  if (IPDUM_TX_CONFIRMATION_API == STD_ON && IPDUM_EXISTS_CONTAINER_PATHWAYTYPEOFTXLOINFO == STD_ON)
      ...
      SchM_Enter_IpduM_IPDUM_EXCLUSIVE_AREA_0();
      {
        ...
        if(bufferInstanceWriteOffset < (IpduM_ContainerTxConfBuIdType)(confirmationBufferInstanceLength - 1))
        {
            ...
            IpduM_IncContainerTxConfBuId(IpduM_GetTxContainerBufferInfoStartIdxOfTxContainerPdu(txContainerId) + currentInstance); /* SBSW_IPDUM_CSL03 */
        }
      }
      SchM_Exit_IpduM_IPDUM_EXCLUSIVE_AREA_0();
#  endif
...
# if (IPDUM_TXCONTAINERPDU == STD_ON) /* COV_IPDUM_VAR_DEPENDENT_SWITCH */
  ...
  if((PduInfoPtr->SduLength >= bufferInstanceLength)
     && ((IpduM_GetContainerWriteCountOfTxContainerBufferIdxInfo(txContainerId) - IpduM_GetContainerReadCountOfTxContainerBufferIdxInfo(txContainerId)) > 0u))
  {
#  if(IPDUM_CONTAINERTXBUFFER==STD_ON) /* COV_IPDUM_VAR_DEPENDENT_SWITCH */
    /* Queue is non-empty */
    SchM_Enter_IpduM_IPDUM_EXCLUSIVE_AREA_0();
...
# if (IPDUM_IMMEDIATEPROCESSINGOFRXCONTAINERPDU == STD_ON)
  ...
  if(IpduM_IsImmediateProcessingOfRxContainerPdu(rxContainerId))
  {
    ...
    SchM_Enter_IpduM_IPDUM_EXCLUSIVE_AREA_0();
    {
      ...
      VStdMemCpyRamToRam(pContainerRxBuf, PduInfoPtr->SduDataPtr, PduInfoPtr->SduLength);       /* SBSW_CONTAINERRXINDICATION_CALL_MEMCPY */
    ...
    IpduM_UnpackRxContainerPdu(rxContainerId, &pduInfo);        /* SBSW_CONTAINERRXINDICATION_CALL_UNPACKRXCONTAINERPDU */
...
# endif
  {
    ...
    if((IpduM_GetContainerWriteCountOfRxContainerBufferIdxInfo(rxContainerId) - IpduM_GetContainerReadCountOfRxContainerBufferIdxInfo(rxContainerId)) >= bufferInstanceCount)
    {
...
# endif
    ...
    /* Enqueue PDU */
    {
      ...
      VStdMemCpyRamToRam(pContainerRxBuf, PduInfoPtr->SduDataPtr, bufferInstanceLength);        /* SBSW_CONTAINERRXINDICATION_CALL_MEMCPY */
...
/* SBSW_JUSTIFICATION_BEGIN
  ...
  \ID SBSW_IPDUM_TRANSMIT_CURR_CONTAINER_PDU_MEMCPY
    ...
    \COUNTERMEASURE \N The second pointer and length always match as they are generated using the ComStackLib.
  ...
  \ID SBSW_ADDTXPDUTOCURRCONTAINER_INDIRECT_ACCESS
    \DESCRIPTION The parameter txContainedId is used as an ID to access generated structures.
    \COUNTERMEASURE \N The caller ensures the txContainedId parameter contains the correct value.
  ...
  \ID SBSW_ADDTXPDUTOCURRCONTAINER_CALL_MEMCPY
    \DESCRIPTION MemCpy is called with two pointers and a length.
  ...
  \ID SBSW_JITUPDATE_CALL_JITTRIGGERTRANSMIT
  ...
  \ID SBSW_ANY_CALL_CONTAINERREADHEADER
  ...
  \ID SBSW_UNPACKRXCONTAINERPDU_CALL_PDURRXINDICATION
  ...
  \ID SBSW_TRIGGERTRANSMIT_CALL_MUXTRIGGERTRANSMIT
  ...
  \ID SBSW_TRIGGERTRANSMIT_CALL_CONTAINERTRIGGERTRANSMIT
  ...
  \ID SBSW_MUXTRIGGERTRANSMIT_CALL_MEMCPY
  ...
  \ID SBSW_MUXTRIGGERTRANSMIT_PDUINFO_WRITE
  ...
  \ID SBSW_CONTAINERTRANSMIT_CALL_MEMCPY
    \DESCRIPTION MemCpy is called with two pointers and a length.
    \COUNTERMEASURE \N The second pointer and length always match as part of PduInfoPtr, the first pointer is
  ...
  \ID SBSW_CONTAINERTRANSMIT_CALL_ADDTXPDUTOCURRCONTAINER
  ...
  \ID SBSW_CONTAINERRXINDICATION_CALL_MEMCPY
    \DESCRIPTION MemCpy is called with two pointers and a length.
    \COUNTERMEASURE \N The second pointer and length always match as part of PduInfoPtr, the first pointer is
                       built using ComStackLib indirections based on the function parameter rxContainerId. The
                       caller verifies this parameter is valid.

  \ID SBSW_CONTAINERRXINDICATION_CALL_UNPACKRXCONTAINERPDU
    \DESCRIPTION IpduM_UnpackRxContainerPdu is called with an ID and a pointer.
