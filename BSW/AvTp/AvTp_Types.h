...
#  define AVTP_AAF_NRS_BIT_POS                   (0U) /* Nominal Sample Rate */
...
#  define AVTP_AAF_EVT_BIT_POS                   (2U) /* Event */
#  define AVTP_AAF_M0_BIT_POS                    (4U) /* M0 */
#  define AVTP_AAF_M1_BIT_POS                    (5U) /* M1 */
#  define AVTP_AAF_M2_BIT_POS                    (6U) /* M2 */
#  define AVTP_AAF_M3_BIT_POS                    (7U) /* M3 */
#  define AVTP_AAF_SP_BIT_POS     AVTP_AAF_M0_BIT_POS
# else
...
#if (AVTP_IEEE1722A_CONFORMANCE == AVTP_IEEE1722A_2014_D8_WITH_D6_AAF_CONFORMANCE)
# define AVTP_AAF_NSR_33_1_3RPM                (0x0AU)
# define AVTP_AAF_NSR_45RPM                    (0x0BU)
# define AVTP_AAF_NSR_78RPM                    (0x0CU)
#endif /* (AVTP_IEEE1722A_CONFORMANCE == AVTP_IEEE1722A_2014_D8_WITH_D6_AAF_CONFORMANCE) */
...
#if (AVTP_IEEE1722A_CONFORMANCE == AVTP_IEEE1722A_2014_D8_WITH_D6_AAF_CONFORMANCE)
# define AVTP_AAF_NRS(param)                    ( (0x0F   & (param)) >> AVTP_AAF_NRS_BIT_POS )
#else
# define AVTP_AAF_NRS(param)                    ( (0xF0   & (param)) >> AVTP_AAF_NRS_BIT_POS )
#endif /* (AVTP_IEEE1722A_CONFORMANCE == AVTP_IEEE1722A_2014_D8_WITH_D6_AAF_CONFORMANCE) */
/* AVTP Audio: Channels Per Frame */
#define AVTP_AAF_CHPERFRAME(high, low)         ( ((0x0300  & (uint16)(high)) << 8 ) | (uint16)(low) )
...
/* AVTP Audio: Event */                       
#if (AVTP_IEEE1722A_CONFORMANCE == AVTP_IEEE1722A_2014_D8_WITH_D6_AAF_CONFORMANCE)
# define AVTP_AAF_EVT(param)                    ( (0x0C  & (param)) >> AVTP_AAF_EVT_BIT_POS )
#else
...
#define AVTP_TIME_SOURCE_ETHTSYN          (0x01U)
#define AVTP_TIME_SOURCE_GPTP             (0x02U)
#define AVTP_TIME_SOURCE_USER_CALLOUT     (0x03U)

#define AVTP_INV_TIME_DOMAIN              (255U) /* Indicates no Time Domain Reference present */
...
typedef struct
{
  ...
  uint8                                   RsvNsr; /* Reserved - Nominal Sample Rate */
  uint8                                   BitDepth; /* Number of actually used bits */
  uint16                                  StreamDataLen; /* Stream Data byte count */
  uint8                                   MEvtChPerFrame_High; /* M3-M0 - Event - Channels per Frame */
  uint8                                   ChPerFrame_Low; /* Low Byte of Channels per Frame */
} AVTP_AAF_MsgType;
# else
typedef struct
{
...
typedef struct
{
  ...
  AvTp_StreamIdType                       StreamId;                       /* StreamId */
#if (STD_ON == AVTP_TIME_SUPPORT)
  uint8                                   TimeDomain;                     /* GPtpPortIdx / EthTSynTimeDomainId */
  uint8                                   TimeSource;
