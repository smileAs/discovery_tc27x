...
#if (STD_ON == AVTP_GPTP_SUPPORT)
#include "GPtp.h"
#endif /* (STD_ON == AVTP_GPTP_SUPPORT) */
#if (STD_ON == AVTP_ETHTSYN_SUPPORT)
#include "EthTSyn.h"
#endif /* (STD_ON == AVTP_ETHTSYN_SUPPORT) */

/**********************************************************************************************************************
 *  VERSION CHECK
 *********************************************************************************************************************/
/* vendor specific version information is BCD coded */
#if (  (AVTP_SW_MAJOR_VERSION != 4u) \
    || (AVTP_SW_MINOR_VERSION != 1u) \
    || (AVTP_SW_PATCH_VERSION != 0u)  )
  #error "Vendor specific version numbers of AvTp.c and AvTp.h are inconsistent"
#endif

/* AUTOSAR version information check has to match definition in header file */
#if (  (AVTP_AR_MAJOR_VERSION != (0x04u)) \
    || (AVTP_AR_MINOR_VERSION != (0x02u)) \
    || (AVTP_AR_PATCH_VERSION != (0x01u))  )
  #error "AUTOSAR Specification Version numbers of AvTp.c and AvTp.h are inconsistent!"
#endif

/* Check configurator version generated to AvTp_Cfg.h */
#if (   (AVTP_CFG_MAJOR_VERSION != (0x04)) \
     || (AVTP_CFG_MINOR_VERSION != (0x01))  )
...
# if (AVTP_IEEE1722A_CONFORMANCE == AVTP_IEEE1722A_2014_D8_WITH_D6_AAF_CONFORMANCE)
      AvTp_AafMsgPtr->RsvNsr = (uint8)(AvTp_CfgGetTxAvtpNsr(StreamIdx) << AVTP_AAF_NRS_BIT_POS);
# else
      AvTp_AafMsgPtr->NsrChPerFrame_High = (uint8)(AvTp_CfgGetTxAvtpNsr(StreamIdx) << AVTP_AAF_NRS_BIT_POS);
# endif /* (AVTP_IEEE1722A_CONFORMANCE == AVTP_IEEE1722A_2014_D8_WITH_D6_AAF_CONFORMANCE) */

      /* set Channels Per Frame */
# if (AVTP_IEEE1722A_CONFORMANCE == AVTP_IEEE1722A_2014_D8_WITH_D6_AAF_CONFORMANCE)
      AvTp_AafMsgPtr->MEvtChPerFrame_High = (uint8)AVTP_AAF_CHPERFRAME_HIGH(AvTp_CfgGetTxAvtpChanPerFram(StreamIdx));
# else
...
# if (AVTP_IEEE1722A_CONFORMANCE == AVTP_IEEE1722A_2014_D8_WITH_D6_AAF_CONFORMANCE)
      /* set Mx flag: Only M0 (Spares Timestamp) used */
      AvTp_AafMsgPtr->MEvtChPerFrame_High |= (uint8)(AvTp_CfgGetTxAvtpSparseMode(StreamIdx) << AVTP_AAF_SP_BIT_POS);
      /* set Event */
      AvTp_AafMsgPtr->MEvtChPerFrame_High |= (AvTp_TxStreamMgmt[StreamIdx].Event << AVTP_AAF_EVT_BIT_POS);
# else
  ...
  uint16 LenByte)
{
  ...
  if(AVTP_INV_TIME_DOMAIN != AvTp_TxStreamInfo[StreamIdx].TimeDomain)
  {
    ...
    /* get current time mod 2^32 */
    (void)AvTp_GetCurrentTime(AvTp_TxStreamInfo[StreamIdx].TimeDomain, &CurrentTime, &SyncState);

    if( (AVTP_PORT_STATE_SYNC == SyncState) || (AVTP_PORT_STATE_UNCERTAIN == SyncState))
    {
      /* calculate: avtp_timestamp = (AS_sec * 10^9 + AS_ns) modulo 2^32 */
      CurrentTime_Mod32 = (AvTp_TimestampType)(((CurrentTime.seconds * ETHTSYN_TIMESTAMP_MAX_NANOSECONDS) +
  ...
  AVTP_P2VAR(AvTp_RxXPduInfoType) AvTp_RxXPduInfoPtr)
{
...
#if (AVTP_IEEE1722A_CONFORMANCE == AVTP_IEEE1722A_2014_D8_WITH_D6_AAF_CONFORMANCE)
  if(AvTp_CfgGetRxAvtpNsr(StreamIdx) != AVTP_AAF_NRS(AafHdrPtr->RsvNsr))
#else
...
#if (AVTP_IEEE1722A_CONFORMANCE == AVTP_IEEE1722A_2014_D8_WITH_D6_AAF_CONFORMANCE)
  if(AvTp_CfgGetRxAvtpChanPerFram(StreamIdx) != AVTP_AAF_CHPERFRAME(AafHdrPtr->MEvtChPerFrame_High,
...
#if (AVTP_IEEE1722A_CONFORMANCE == AVTP_IEEE1722A_2014_D8_WITH_D6_AAF_CONFORMANCE)
  if((uint8)(AvTp_CfgGetRxAvtpSparseMode(StreamIdx)) != AVTP_AAF_SP(AafHdrPtr->MEvtChPerFrame_High))
#else
  ...
  /* get Event */
#if (AVTP_IEEE1722A_CONFORMANCE == AVTP_IEEE1722A_2014_D8_WITH_D6_AAF_CONFORMANCE)
  AafInfoPtr->Event = (uint8)AVTP_AAF_EVT(AafHdrPtr->MEvtChPerFrame_High);
#else
