...
STATIC FUNC(void, TCPIP_CODE) TcpIp_Tcp_VDropEndEleInRetrQueue(
    TcpIp_TcpIdxType TcpIdx,
    uint8 NumDropEle );
STATIC FUNC(void, TCPIP_CODE) TcpIp_Tcp_VRemoveEleInRetrQueue(
  TcpIp_TcpIdxType TcpIdx,
  TcpIp_TcpTxRetrQueueIdxType DropEleIdx );
    ...
    )
{
  ...
  TcpIp_CheckDetErrorReturnVoid( TCPIP_COND_VALID_PTR(RxSockRemAddrPtr), TCPIP_TCP_API_ID_RX_INDICATION, TCPIP_E_PARAM_POINTER )
  TcpIp_CheckDetErrorReturnVoid( TCPIP_COND_VALID_PTR(DataPtr), TCPIP_TCP_API_ID_RX_INDICATION, TCPIP_E_PARAM_POINTER )
  /* ------------------------------------------------- */
 ...
 *  \return      TRUE                   socket is connected <br>
...
#if (TCPIP_SUPPORT_TLS == STD_ON)
  /* The current TLS implementation does not support server sockets, so this case should never happen.
     -> do nothing */
#else
    ...
    uint8 Security */ )
{
  ...
  TcpIp_CheckDetErrorReturnValue( TCPIP_COND_VALID_PTR(SockAddrPtr), TCPIP_TCP_API_ID_OPEN, TCPIP_E_PARAM_POINTER, TCPIP_TCP_INV_IDX )
  /* ------------------------------------------------- */
...
# endif
      {
        retValue = IpV4_Ip_Transmit(sockIdx, &ipTxReqestDescr, (uint16)dataLenByte, TCPIP_PROTOCOL_TCP, ((IpBase_SockAddrInType*)&(TcpIp_SocketList[sockIdx].RemSock))->sin_addr, FALSE, 0, FALSE);  /* PRQA S 0310 */ /* MD_TCPIP_11.4_0310_sock */
      }
# if (TCPIP_SUPPORT_IPV6 == STD_ON)
      else
# endif
...
#if (TCPIP_SUPPORT_IPV6 == STD_ON)
      {
        retValue = IpV6_Transmit(sockIdx, &ipTxReqestDescr, TCPIP_PROTOCOL_TCP, (uint16)dataLenByte, FALSE, 0, FALSE);
      }
#endif
    }
  }
...
/* ----- end check if the segment shall be sent ------------------------------------------------- */
  ...
  TCPIP_VENTER_CRITICAL_SECTION();  /* PRQA S 3109 */ /* MD_MSR_14.3 */
  {
    /* go backwards through all elements of the retry queue, remove already sent empty ACKs from the list */
    RetrQueueEleIdx = TcpIp_Tcp_LastRetrQueueEle[TcpIdx];
    while(RetrQueueEleIdx != TCPIP_TCP_TX_RETR_QUEUE_INV_IDX)
    {
      TcpIp_Tcp_TxRetrQueueType   *TxRetrQueueElemPtr;
      TcpIp_TcpTxRetrQueueIdxType PrevEle;

      TxRetrQueueElemPtr = &TcpIp_Tcp_TxRetrQueue[RetrQueueEleIdx];
      PrevEle = TxRetrQueueElemPtr->ElePrev;
      if((TxRetrQueueElemPtr->SizeTotByte == 0) && ((TxRetrQueueElemPtr->Flags & (TCPIP_TCP_CODE_BIT_SYN | TCPIP_TCP_CODE_BIT_FIN)) == 0) && (TxRetrQueueElemPtr->TransmitCounter > 0))
      {
        /* remove this last element from list */
        TcpIp_Tcp_VRemoveEleInRetrQueue(TcpIdx, RetrQueueEleIdx);
      }
      else
      {
        /* skip this element */
...
STATIC FUNC(void, TCPIP_CODE) TcpIp_Tcp_VTransmitResets( void )
{
  ...
  while(0 != TcpIp_Tcp_RstTxQueueFillNum)
  {
    ...
    {
      /* V4 */
      RetValue = IpV4_Ip_Transmit(SockIdx, &IpTxReqestDescr, TCPIP_TCP_HDR_LEN_BYTE, TCPIP_PROTOCOL_TCP, ((IpBase_SockAddrInType*)&(RstQueuePtr->RemSock))->sin_addr, FALSE, 0, FALSE);  /* PRQA S 0310 */ /* MD_TCPIP_11.4_0310_sock */
    }
#if (TCPIP_SUPPORT_IPV6 == STD_ON)
    else
#endif
    ...
    {
      /* V6 */
      RetValue = IpV6_Transmit(SockIdx, &IpTxReqestDescr, TCPIP_PROTOCOL_TCP, TCPIP_TCP_HDR_LEN_BYTE, FALSE, 0, FALSE);
    }
#endif
 ...
 **********************************************************************************************************************/
/*! \brief        Remove an element from the socket's TxRetrQueue
 *  \param[in]    TcpIdx               TCP index
 *  \param[in]    DropEleIdx           Index of the element that shall be removed
 *  \context      interrupt or task level
 *  \note         It must be assuerd that the given element is part of the socket's TxRetrQueue.
 **********************************************************************************************************************/
STATIC FUNC(void, TCPIP_CODE) TcpIp_Tcp_VRemoveEleInRetrQueue(
  TcpIp_TcpIdxType TcpIdx,
  TcpIp_TcpTxRetrQueueIdxType DropEleIdx )
{
  ...
  if(0 == TcpIp_Tcp_TxRetrQueueFillNum[TcpIdx])
  {
    /* error, there is no element in the list */
    TCPIP_VLEAVE_CRITICAL_SECTION();  /* PRQA S 3109 */ /* MD_MSR_14.3 */
    return;
  }

  /* remove element from the socket's TxRetrQueue --------------------------- */
  if(1 == TcpIp_Tcp_TxRetrQueueFillNum[TcpIdx])
  {
    /* there is only one element in the queue, so the queue will be empty afterwards */
    TcpIp_Tcp_FirstRetrQueueEle[TcpIdx]  = TCPIP_TCP_TX_RETR_QUEUE_INV_IDX;
    TcpIp_Tcp_LastRetrQueueEle[TcpIdx]   = TCPIP_TCP_TX_RETR_QUEUE_INV_IDX;
    TcpIp_Tcp_TxRetrQueueFillNum[TcpIdx] = 0;
  }
  else
  {
    ...
    TcpIp_TcpTxRetrQueueIdxType PredecessorEleIdx = TcpIp_Tcp_TxRetrQueue[DropEleIdx].ElePrev;
    TcpIp_TcpTxRetrQueueIdxType SuccessorEleIdx   = TcpIp_Tcp_TxRetrQueue[DropEleIdx].EleNext;

    if(TcpIp_Tcp_FirstRetrQueueEle[TcpIdx] == DropEleIdx)
    {
      /* remove first segment of the retry queue */
      TcpIp_Tcp_TxRetrQueue[SuccessorEleIdx].ElePrev   = TCPIP_TCP_TX_RETR_QUEUE_INV_IDX;
      TcpIp_Tcp_FirstRetrQueueEle[TcpIdx]              = SuccessorEleIdx;
    }
    else if(TcpIp_Tcp_LastRetrQueueEle[TcpIdx] == DropEleIdx)
    {
      /* remove last segment of the retry queue */
      TcpIp_Tcp_TxRetrQueue[PredecessorEleIdx].EleNext = TCPIP_TCP_TX_RETR_QUEUE_INV_IDX;
      TcpIp_Tcp_LastRetrQueueEle[TcpIdx]               = PredecessorEleIdx;
    }
    else
    {
      /* remove segment somewhere in the middle of the retry queue */
      TcpIp_Tcp_TxRetrQueue[PredecessorEleIdx].EleNext = SuccessorEleIdx;
      TcpIp_Tcp_TxRetrQueue[SuccessorEleIdx].ElePrev   = PredecessorEleIdx;
    }
    TcpIp_Tcp_TxRetrQueueFillNum[TcpIdx]--;
  }

  /* now apend the removed element to the FreeList -------------------------- */
  if(0 == TcpIp_Tcp_NumFreeRetrQueueEle)
  {
    /* FreeList has been empty */
    TcpIp_Tcp_TxRetrQueue[DropEleIdx].ElePrev = TCPIP_TCP_TX_RETR_QUEUE_INV_IDX;
    TcpIp_Tcp_TxRetrQueue[DropEleIdx].EleNext = TCPIP_TCP_TX_RETR_QUEUE_INV_IDX;
    TcpIp_Tcp_FirstFreeRetrQueueEle    = DropEleIdx;
    TcpIp_Tcp_LastFreeRetrQueueEle     = DropEleIdx;
  }
  else
  {
    TcpIp_Tcp_TxRetrQueue[TcpIp_Tcp_LastFreeRetrQueueEle].EleNext = DropEleIdx;
    TcpIp_Tcp_TxRetrQueue[DropEleIdx                    ].ElePrev = TcpIp_Tcp_LastFreeRetrQueueEle;
    TcpIp_Tcp_TxRetrQueue[DropEleIdx                    ].EleNext = TCPIP_TCP_TX_RETR_QUEUE_INV_IDX;
    TcpIp_Tcp_LastFreeRetrQueueEle     = DropEleIdx;
  }
  TcpIp_Tcp_NumFreeRetrQueueEle++;
 ...
 *              <br>If there are spare retry queue elements, the first element of the connections retry queue is split
