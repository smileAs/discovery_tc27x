...
#if (  (IPV6_SW_MAJOR_VERSION != 5u) \
    || (IPV6_SW_MINOR_VERSION != 0u) \
 ...
 **********************************************************************************************************************/
/*! \brief      This method compares two IPv6 addresses
 *  \param[in]  LLAddr1Ptr   first address
 *  \param[in]  LLAddr2Ptr   second address
 *
 *  \return     TRUE         addresses are equal
 *  \return     FALSE        addresses are not equal
 **********************************************************************************************************************/
FUNC(boolean, IPV6_CODE) IpV6_CmpLLAddr(
  P2CONST(uint8, AUTOMATIC, IPV6_APPL_DATA) LLAddr1Ptr,
  P2CONST(uint8, AUTOMATIC, IPV6_APPL_DATA) LLAddr2Ptr)
{

#if (IPV6_VENABLE_INTERNAL_DET_CHECKS == STD_ON)
  /* PRQA S 3109 2 */ /* MD_MSR_14.3 */
  IpV6_CheckDetErrorReturnValue(NULL_PTR != LLAddr1Ptr, IPV6_API_ID_V_INTERNAL_FUNCTION, IPV6_E_INV_POINTER, FALSE);
  IpV6_CheckDetErrorReturnValue(NULL_PTR != LLAddr2Ptr, IPV6_API_ID_V_INTERNAL_FUNCTION, IPV6_E_INV_POINTER, FALSE);
#endif
 ...
 *  \param[out] SrcAddrStatePtr specifies thet status of the source address
...
#if (IPV6_ENABLE_PRIVACY_EXTENSIONS == STD_ON)
  if (E_OK == IpV6_VSATLookup(IpCtrlIdx, SrcAddrPtr, &CtrlAddrIdx, &SatTempAddrIdx))
#else
  if (E_OK == IpV6_VSATLookup(IpCtrlIdx, SrcAddrPtr, &CtrlAddrIdx))
#endif
  {
    IpV6_LocAddrStateType SrcAddrState;
...
#endif
    {
      SrcAddrState = IPV6_CTRL_VAR(IpV6_SourceAddressTable)[CtrlAddrIdx].State;
    }
  ...
  # endif
      {
         ...
         * We have to consider only the first non-tentative address here, since the older entires will not result in better matches.
 ...
 **********************************************************************************************************************/
FUNC(void, IPV6_CODE) IpV6_VHandleDADFailure(IpV6_AddrIdType AddrId, uint8 EventType)
{
 ...
 *  IpV6_GetVersionInfo()
 **********************************************************************************************************************/
/*! \brief      Get version of the IpV6 module
 *  \param[out] VersionInfoPtr        pointer to version information structure
 *  \context    task level
 **********************************************************************************************************************/
#if (IPV6_VERSION_INFO_API == STD_ON)
FUNC(void, IPV6_CODE) IpV6_GetVersionInfo(
  P2VAR(Std_VersionInfoType, AUTOMATIC, IPV6_APPL_DATA) VersionInfoPtr)
{
  /* Det checks */
  /* PRQA S 3109 1 */ /* MD_MSR_14.3 */
  IpV6_CheckDetErrorReturnVoid((NULL_PTR != VersionInfoPtr), IPV6_API_ID_GET_VERSION_INFO, IPV6_E_INV_POINTER);
  /**/

  VersionInfoPtr->vendorID = IPV6_VENDOR_ID;
  VersionInfoPtr->moduleID = IPV6_MODULE_ID;
  VersionInfoPtr->sw_major_version = IPV6_SW_MAJOR_VERSION;
  VersionInfoPtr->sw_minor_version = IPV6_SW_MINOR_VERSION;
  VersionInfoPtr->sw_patch_version = IPV6_SW_PATCH_VERSION;

  /* PRQA S 2006 1 */ /* MD_MSR_14.7 */
} /* End of IpV6_GetVersionInfo() */
#endif /* IPV6_VERSION_INFO_API == STD_ON */
 ...
 *              IpV6_ProvideTxBuffer().
 *  \param[in]  SockIdx        TcpIp socket index
    ...
    IPV6_P2V(TcpIp_IpTxRequestDescriptorType) TxReqDescrPtr) /* PRQA S 3673 */ /* MD_IPV4_16.7_3673 */
{
  ...
  IPV6_UNUSED_PARAMETER(SockIdx);  /* PRQA S 3112, 3199 */ /* MD_MSR_14.2 */

  /* Det checks */
  /* PRQA S 3109 3 */ /* MD_MSR_14.3 */
  IpV6_CheckDetErrorReturnVoid(IPV6_IS_INITIALIZED,             IPV6_API_ID_CANCEL_TRANSMIT, IPV6_E_NOT_INITIALIZED);
  IpV6_CheckDetErrorReturnVoid(IPV6_IS_VALID_SOCK_IDX(SockIdx), IPV6_API_ID_CANCEL_TRANSMIT, IPV6_E_INV_SOCK_IDX);
  ...
  uint16 LenByte)
{
...
#if(TCPIP_TCPIP_HW_BASED_CHECKSUM_ENABLED == STD_ON)
  boolean HwChecksumCheckPossible = TRUE;   /* summarize information if the hardware could calculate the upper layer checksum */
#endif

  IPV6_UNUSED_PARAMETER(PhysAddrPtr);  /* PRQA S 3112, 3199 */ /* MD_MSR_14.2 */
  ...
  if (TRUE == IpV6_VIsValidSourceAddress(IpCtrlIdx, &IpV6HdrPtr->SrcAddr, FALSE, &SrcAddrState))
  {
#if (IPV6_ENABLE_OPTIMISTIC_DAD == STD_ON)
...
#endif
    {
      ...
      return; /* Silently ignore packet sent from an address that is assigned to this node. (packet is sent by this node) */
    }
  }
  ...
  while (IPV6_HDR_ID_NO_NEXT_HDR != NextHdrValue)
  {
    ...
    case IPV6_EXT_HDR_ID_AUTH:
      ...
      ThisHdrLen = 0;
#if(TCPIP_TCPIP_HW_BASED_CHECKSUM_ENABLED == STD_ON)
      HwChecksumCheckPossible = FALSE;  /* if IPSec is used no upper layer checksums can be calculated in HW */
#endif
      break;
    ...
    case IPV6_HDR_ID_ICMP:
      ...
      IpV6_Icmp_VPacketReceived(AddrId, IpV6HdrPtr, DataPtr, ThisHdrOfs, DataLen
#if (TCPIP_TCPIP_HW_BASED_CHECKSUM_ENABLED == STD_ON)
        , HwChecksumCheckPossible
#endif
        );
      NextHdrValue = IPV6_HDR_ID_NO_NEXT_HDR;
      ThisHdrLen = 0;
...
#endif
        {
          TcpIp_Udp_RxIndication(IPV6_ADDR_ID_2_TCPIP_LOCAL_ADDR_ID(AddrId),
  ...
  IPV6_P2C(IpV6_AddrType) DstAddrPtr)
{
  ...
  if (TRUE == IpV6_VIsValidSourceAddress(IpCtrlIdx, DstAddrPtr, TRUE, &SrcAddrState))
  {
