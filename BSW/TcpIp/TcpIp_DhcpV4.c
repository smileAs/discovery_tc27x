...
FUNC(void, TCPIP_CODE) TcpIp_DhcpV4_MainFunction(void)
{
  ...
  /* ------------------------------------------------- */
  /* DET: */
  TcpIp_CheckDetErrorReturnVoid( TCPIP_COND_INIT, TCPIP_DHCPV4_API_ID_MAIN_FUNCTION, TCPIP_E_NOTINIT )
  /* ------------------------------------------------- */
...
#else
  {
    ...
    /* #30 Update time varaible used for timeout handling. */
    ...
    {
      /* -------------------------------------------------------------------- */
      /* start of STATE dependend processing -------------------------------- */
    ...
    TcpIp_NetAddrType Ciaddr )
{
  /* ------------------------------------------------- */
  /* DET: */
  TcpIp_CheckDetErrorReturnVoid( TCPIP_COND_INIT, TCPIP_DHCPV4_API_ID_TRIGGER_ADDR_REQU, TCPIP_E_NOTINIT )
  /* ------------------------------------------------- */
    ...
    uint8 IpCtrlIdx )
{
  /* ------------------------------------------------- */
  /* DET: */
  TcpIp_CheckDetErrorReturnVoid( TCPIP_COND_INIT, TCPIP_DHCPV4_API_ID_STOP_ADDR_REQU, TCPIP_E_NOTINIT )
  /* ------------------------------------------------- */
        ...
        )
{
  ...
  /* ------------------------------------------------- */
  /* DET: */
  TcpIp_CheckDetErrorReturnVoid( TCPIP_COND_INIT,                   TCPIP_DHCPV4_API_ID_RX_INDICATION, TCPIP_E_NOTINIT )
  TcpIp_CheckDetErrorReturnVoid( TCPIP_COND_VALID_PTR(DataPtr),     TCPIP_DHCPV4_API_ID_RX_INDICATION, TCPIP_E_PARAM_POINTER )
  /* ------------------------------------------------- */
  ...
  if (IPV4_HTONS(UdpHdrPtr->LenByte) > DataLen)
  {
    return;
  }
  ...
  DataLen = IPV4_HTONS(UdpHdrPtr->LenByte) - 8;

  FramePtr = (TcpIp_DhcpV4_FrameType *)(&DataPtr[0]);  /* PRQA S 0310 */ /* MD_TCPIP_11.4_0310_struct */

  /* find the corresponing controller */
  /* compare Msg-HW-Addr with own addr (future: compare to all controller addresses) */
  TCPIP_LL_GetPhysAddr( IpCtrlIdx, &PhysAddr[0] );
  if( (TRUE == TcpIp_ComparePhysAddr(FramePtr->Chaddr, &PhysAddr[0])) &&  /* message is for this node */
      (TCPIP_DHCPV4_OP_BOOTREPLY == FramePtr->Op) &&                              /* this message comes from a server */
      (FramePtr->Xid == TcpIp_DhcpV4_RAM_DataStruct(IpCtrlIdx).Xid) )               /* this message corresonds to the current DHCP XID */
  {
            ...
            case(TCPIP_DHCPV4_OPT_DOMAIN_NAME_SERVER_TYPE):
              {
                ...
                {
                  IPV4_GET_UINT32_HBO(OptPtr, 2, DnsServerOpt); /* Address is stored in host byte-order */
        ...
        {
          /* invalid DHCP message type, or invalide lease time */
          MsgIsValid = FALSE;
        }
        ...
        {
          /* discard messages from other servers than the selcted one */
          MsgIsValid = FALSE;
        }
        ...
        if(TRUE == MsgIsValid)
        {
          if(TCPIP_DHCPV4_CTRL_STATE_SELECTING == TcpIp_DhcpV4_RAM_CtrlState(IpCtrlIdx))
          {
            ...
            if(TCPIP_DHCPV4_DHCPACK == MsgType)
            {
              /* ignore incoming DHCPACK in this state */
            }
          }
          ...
          else if(TCPIP_DHCPV4_CTRL_STATE_BOUND == TcpIp_DhcpV4_RAM_CtrlState(IpCtrlIdx))
          {
            if( (TCPIP_DHCPV4_DHCPOFFER == MsgType) ||
                (TCPIP_DHCPV4_DHCPACK == MsgType) ||
                (TCPIP_DHCPV4_DHCPNACK == MsgType) )
            {
              /* ignore this DHCP message */
            }
          }
          else
          {
    ...
    uint8 IpCtrlIdx )
{
  /* ------------------------------------------------- */
  /* DET: */
  TcpIp_CheckDetErrorReturnVoid( TCPIP_COND_INIT, TCPIP_DHCPV4_API_ID_RESET, TCPIP_E_NOTINIT )
  /* ------------------------------------------------- */
    ...
    uint8 HostNameLenByte )
{
  /* ------------------------------------------------- */
  /* DET: */
  TcpIp_CheckDetErrorReturnValue( TCPIP_COND_VALID_PTR(HostNamePtr), TCPIP_DHCPV4_API_ID_SET_HOST_NAME, TCPIP_E_PARAM_POINTER, E_NOT_OK )
    ...
    P2VAR(IpV4_IpAddrSetType, AUTOMATIC, TCPIP_APPL_DATA) addrInfoPtr)
{
  ...
  IPV4_ASSERT(IpV4_Ip_VCfgIsAddrAssignmentEnabled(IPV4_IP_ADDR_ASSIGNMENT_METHOD_DHCP));
...
# endif
    ...
    /* send data */
    if(TCPIP_OK == IpV4_Ip_Transmit((TcpIp_SockIdxType)TcpIp_DhcpV4_SockHnd, &IpTxReqestDescr, UdpLenByte, TCPIP_PROTOCOL_UDP, Dest.sin_addr, FALSE, 0, FALSE))
    {
 ...
 **********************************************************************************************************************/
/*! \brief      Get user option data for outgoing DHCPv4 messages. <br>
