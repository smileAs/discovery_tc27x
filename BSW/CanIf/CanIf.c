...
#if (CANIF_CBK_MINOR_VERSION != 0x02u)
# error "Source and Header files are inconsistent!"
#endif

#if (CANIF_CBK_PATCH_VERSION != 0x01u)
# error "Source and Header files are inconsistent!"
#endif
...
#if (CANIF_TYPES_MINOR_VERSION != 0x13u)
# error "Source and Header files are inconsistent!"
#endif

#if (CANIF_TYPES_PATCH_VERSION != 0x0u)
# error "Source and Header files are inconsistent!"
#endif
...
#if (CANIF_API_MINOR_VERSION != 0x13u)
# error "Source and Header files are inconsistent!"
#endif
...
/*! \def Define CANIF_WU_VALIDATION_NM_MASK_OUT
...
#define CANIF_GENDATA_COMPATIBILITY_VERSION     0x0211u
...
#define CANIF_NO_ENTRY_IN_TX_FIFO            (CanIf_TxBufferSizeType)0xFFFFFFFFu
...
#if ((CANIF_SUPPORT_NMOSEK_INDICATION == STD_ON) || (CANIF_META_DATA_RX_SUPPORT == STD_ON))   
 ...
 *  \param[in]   CanId: CAN identifier (optional, depending on configuration)
 *  \pre         -
 *  \context     ANY
 *  \reentrant   FALSE
 *  \synchronous TRUE
 *  \config      1. CANIF_SUPPORT_NMOSEK_INDICATION == STD_ON or CANIF_META_DATA_RX_SUPPORT == STD_ON
 *               2. !1.
 **********************************************************************************************************************/
#if ((CANIF_SUPPORT_NMOSEK_INDICATION == STD_ON) || (CANIF_META_DATA_RX_SUPPORT == STD_ON))
 ...
 **********************************************************************************************************************/
/*! \brief       Modifies the CAN identifier according to J1939 specification.
 *  \details     Called out of CanIf_RxIndication().
 ...
 *  \details     Is called out from CanIf_RxIndicationSubRangeSearch() and CanIf_RxIndication().
 *  \param[in]   rxSearchParams: Parameters needed for search.
 ...
 *  \details     This API uses the search algorithm linear. Called out of CanIf_RxIndication().
 *  \param[in]   rxSearchParams: Parameters needed for search.
 *  \pre         -
 *  \context     ANY
 *  \reentrant   TRUE
...
} /* PRQA S 6080 */ /* MD_CanIf_14.7,MD_MSR_STMIF */
...
# if ((CANIF_DEV_ERROR_DETECT == STD_ON ) || (CANIF_CONFIG_VARIANT == CANIF_CFGVAR_POSTBUILDTIME)) /* COV_CANIF_DEV_ERROR_DETECT_ALWAYS_TRUE */
...
} /* PRQA S 2006,6010,6080,6030 */ /* MD_CanIf_14.7,MD_MSR_STPTH,MD_MSR_STMIF,MD_MSR_STCYC*/
...
# else
  {
    ...
    {
#  if (CANIF_STATIC_FD_TXQUEUE == STD_ON)
      CanIf_SetTxQueueData((CANIF_CFG_TXBUFFERPRIOBYCANID_STATFDQUEUE_DATASTARTIDX(PduInfoPtr->swPduHandle))+len, PduInfoPtr->sdu[len]); /* SBSW_CANIF_40 */
#  else
      CANIF_VAR_TXBUFFERPRIOBYCANID_QUEUE(queueindex, eQueueParams.eSdu[len]) = PduInfoPtr->sdu[len]; /* SBSW_CANIF_6 */
#  endif
    }
  }
...
CANIF_LOCAL FUNC(void, CANIF_CODE) CanIf_HlTxConfirmation(PduIdType CanTxPduId)
{
  ...
  if (txConfirmationFct != NULL_PTR) /* COV_CANIF_CONFLICTS_TESTENVIROMENT_FAILURE */
  {
...
# endif        
#endif
      {
        ...
        if (CANIF_CFG_MAX_RXPDUS != 0u)  /* COV_CANIF_CONFLICTS_TESTENVIROMENT_FAILURE */
        {
...
#endif
{
 ...
 {
   /* #400 CanIf_AdvancedRxIndicationLayout API */
...
#if (CANIF_DEV_ERROR_DETECT == STD_ON)
# if (CANIF_ONE_CONTROLLER_OPTIMIZATION == STD_OFF) /* COV_CANIF_ONE_CHANNEL_OPTIMIZATION_UNSUPPORTED */
  /* #20 Check if CAN channel is valid */
...
#  if (CANIF_WAKEUP_VALID_ONLY_NM_RX_MSGS == STD_ON)
      if((uint8)(CANIF_CFG_RX_DLC(PduId) & (uint8)(~CANIF_WU_VALIDATION_NM_MASK_OUT)) != 0)
#  endif
      {
...
# endif
    ...
    /* #30 Check if affected CAN channel is initialized */
    else if (CanIf_GetCtrlMode(CanIf_Controller_Value) == CANIF_CS_UNINIT)
    {
...
#endif
    {
      ...
      /* Requirement is sourced out of sequence diagram for BusOff notification */
...
#if (CANIF_DEV_ERROR_DETECT == STD_OFF)
      /* \trace SPEC-3705 */
      if(CANIF_CS_UNINIT != CanIf_GetCtrlMode(CanIf_Controller_Value)) /* COV_CANIF_HARDWARE_FAILURE */ /* cannot be stimulated --> controllermode set to STOP (3 instructions before) */
#endif
      {
        /* \trace SPEC-5346 */

        /* #130 If CAN channel is initialized AND an indication function to upper layer is configured inform the upper layer */
        busoffnotificationFct = CANIF_CFG_BUSOFFNOTIFICATION();

        if (busoffnotificationFct != NULL_PTR) /* COV_CANIF_CONFLICTS_TESTENVIROMENT_FAILURE */
        {
...
# endif
    ...
    if (txResult == CAN_OK)  /* COV_CANIF_HARDWARE_FAILURE */
    {
      /* \trace SPEC-1897 */
...
# endif        
    ...
    else if (txResult == CAN_BUSY) /* COV_CANIF_HARDWARE_FAILURE */ /* PRQA S 2004 */ /* MD_CanIf_14.4 */
    {
...
#  if (CANIF_BITQUEUE == STD_ON)
      ...
      if (CanIf_GetTxQueueCounter(txBufferPrioByCanIdBaseIdx) != 0u)
      {
        ...
        { 
          ...
          if(elem != (CanIf_TxQueueFlagType)0) /* is there any flag set in the queue element? */  /* COV_CANIF_COVERAGE_DEPENDS_ON_VARIANT */
          {
            /* iterate through all flags and notify application for every scheduled transmission */
            for(elementBitIdx = ((sint8)1 << kCanTxQueueShift) - (sint8)1; elementBitIdx >= (sint8)0; elementBitIdx--)  /* COV_CANIF_DIVIDED_DECISION */
            { 
...
#  else
      ...
      if (CanIf_GetTxQueueCounter(txBufferPrioByCanIdBaseIdx) != 0u)
      {
        ...
        for (idx2MappedTxPdus = (CanIf_AnyIdxType)(CANIF_CFG_TXBUFFERPRIOBYCANIDBYTEQUEUE_STOPIDX2MAPPEDTXPDUS(txBufferCfgIdx)-1); idx2MappedTxPdus >= CANIF_CFG_TXBUFFERPRIOBYCANIDBYTEQUEUE_STARTIDX2MAPPEDTXPDUS(txBufferCfgIdx); idx2MappedTxPdus--) /* COV_CANIF_DIVIDED_DECISION */      
        {
          CanIf_AnyIdxType TxPduId;
...
FUNC(Std_ReturnType, CANIF_CODE) CanIf_SetTrcvMode(uint8 TransceiverId, CanTrcv_TrcvModeType TransceiverMode)
{
  ...
  {
    /* ----- Local Variables ---------------------------------------------- */
# if (CANIF_DEV_ERROR_DETECT == STD_ON) /* COV_CANIF_DEV_ERROR_DETECT_ALWAYS_TRUE */
    CANIF_CHANNEL_CANTYPE_LOCAL /* PRQA S 0781 */ /* MD_CanIf_5.6 */
# endif
    uint8 errorId = CANIF_E_NO_ERROR;
    retval = E_NOT_OK; /* PRQA S 3198 */ /* MD_MSR_14.2 */
...
# endif
      {
# if (CANIF_DEV_ERROR_DETECT == STD_OFF) /* COV_CANIF_DEV_ERROR_DETECT_ALWAYS_TRUE */
        /* \trace SPEC-5334 */
        if (CanIf_GetCtrlMode(CanIf_Controller_Value_Local) != CANIF_CS_UNINIT)
# endif
        {
...
# endif
    ...
    localRetVal = E_NOT_OK; /* PRQA S 3198 */ /* MD_MSR_14.2 */
...
# if (CANIF_TRCV_MAPPING == STD_ON)
        ...
        {
          /* #110 Pass the request to the transceiver driver */
          localRetVal = lowFct(TrcvChannelId);  /* SBSW_CANIF_22 */
        }
        else
        {
          errorId = CANIF_E_PARAM_TRCV;
...
# else
        /* \trace SPEC-9964 */
        localRetVal = CanTrcv_ClearTrcvWufFlag(TransceiverId);
# endif
      }
    } /* Valid TransceiverId */
...
# else
        localRetVal = CanTrcv_CheckWakeFlag(TransceiverId);
# endif /* (CANIF_TRCV_MAPPING == STD_ON) */
      }
    } /* Valid TransceiverId */
...
# if ((CANIF_DEV_ERROR_DETECT == STD_ON) && (CANIF_USE_INIT_POINTER == STD_ON))
    /* #130 Check if CanIf is initialized at all */
    if (CanIf_SystemInit == FALSE)
    {
      retval = E_NOT_OK;
      /* \trace SPEC-5238 */
...
#  if (CANIF_ONE_CONTROLLER_OPTIMIZATION == STD_OFF) /* COV_CANIF_ONE_CHANNEL_OPTIMIZATION_UNSUPPORTED */
        /* #140 Check the CAN channel parameter */
...
#  endif /* CANIF_ONE_CONTROLLER_OPTIMIZATION == STD_OFF */
        /* #150 Check if CAN channel is initialized */
...
# endif /* CANIF_DEV_ERROR_DETECT == STD_ON */
        {
          /* #160 Is it the wake-up source which shall be checked for wake-up? */
          if ((WakeupSource & wuSrc) != 0)
          {
            /* \trace SPEC-5284 */
            /* #170 Process only if CAN channel is either in SLEEP or in STOP */
            if ((CanIf_GetCtrlMode(CanIf_WU_Controller_Value_Local) == CANIF_CS_SLEEP) || (CanIf_GetCtrlMode(CanIf_WU_Controller_Value_Local) == CANIF_CS_STOPPED))
            {
              ...
              if (wuTrgtmodule == CANIF_WAKEUPREQUEST_CAN) /* COV_CANIF_GENDATA_FAILURE */
              {
...
# if (CANIF_TRCV_HANDLING == STD_ON)
              ...
              else if (wuTrgtmodule == CANIF_WAKEUPREQUEST_TRCV) /* COV_CANIF_GENDATA_FAILURE */
              {
...
# endif /* CANIF_TRCV_HANDLING == STD_ON */
              ...
              if (localRetval == E_OK)
              {
                /* #500 Mark if the wake-up request was confirmed by CAN channel / transceiver channel */
    ...
    {
      /* #510 If no wake-up sources were confirmed return E_NOT_OK */
      retval = E_NOT_OK;
    }
...
{
  CanIf_NotifStatusType eLocalRetVal;
  {
    /* ----- Local Variables ---------------------------------------------- */
    uint8 errorId = CANIF_E_NO_ERROR;

    eLocalRetVal = CANIF_NO_NOTIFICATION; /* PRQA S 3198 */ /* MD_MSR_14.2 */
...
# endif /* (CANIF_DEV_ERROR_DETECT == STD_ON) */
    {
      ...
      eLocalRetVal = CanIf_GetTxConfState(CanIf_Controller_Value);
    }
...
{
  {
    CanIf_RamCheckCorruptMailboxIndFctType ramCheckCorruptMailboxIndFct;
    ramCheckCorruptMailboxIndFct = CANIF_CFG_RAMCHECKCORRUPTMAILBOXINDFCT();

    /* #10 Check if function pointer is valid */
...
{
  {
    CanIf_RamCheckCorruptControllerIndFctType ramCheckCorruptControllerIndFct;
    ramCheckCorruptControllerIndFct = CANIF_CFG_RAMCHECKCORRUPTCONTROLLERINDFCT();
...
MD_CanIf_14.7
  Reason: Multiple return paths used to optimize code (readability and runtime).
  Risk: Return from a function too soon.
  Prevention: Code inspection.
...
\ID COV_CANIF_CONFLICTS_TESTENVIROMENT_FAILURE
  \ACCEPT TX
  \ACCEPT XX
  \REASON If the condition is inverted, the test suite does not work. The code is verified by code inspection.
  
\ID COV_CANIF_HARDWARE_FAILURE
  \ACCEPT TX
  \ACCEPT XF
  \ACCEPT XX
...
\ID COV_CANIF_UNSUPPORTED_DYN_ADDR
  \ACCEPT TF tf tx tf
  \ACCEPT TF tf xf tf
...
\ID COV_CANIF_DIVIDED_DECISION
  \ACCEPT TX
  \REASON Loop is not able to end via its end statement, the break statement in the loop body matches at any event due to previous TxQueueCounter != 0 - check!
...
\ID COV_CANIF_COVERAGE_DEPENDS_ON_VARIANT
  \ACCEPT TX
  \REASON If the decision is able to get false depends on the number of Tx Pdus and out of this can not be varied within one configuration.\
          It will be secured by inspection that both decisions are covered, for minimum one time, over all configurations.

\ID COV_CANIF_DEV_ERROR_DETECT_ALWAYS_TRUE
  \ACCEPT TX tf tf
  \ACCEPT TX
  \ACCEPT XF xf xf
  \REASON For SafeBSW configs, the switch CANIF_DEV_ERROR_DETECT must be always STD_ON
