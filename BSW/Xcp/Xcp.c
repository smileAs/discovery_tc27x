...
VAR(Xcp_ChannelStruct, XCP_VAR_NOINIT) Xcp_ChannelInfo[XCP_NUMBER_OF_CHANNELS];
 ...
 * \details     Sends the next pending event message or text.
 * \param[in]   XcpChannel             The channel from which the data should be send.
 * \return      FALSE                  The TxConfirmation did not occur while waiting, new message must be rejected.
 * \return      TRUE                   The TxConfirmation occurred while waiting, new message can be sent.
 * \context     TASK
 * \reentrant   FALSE
 * \synchronous TRUE
 * \config      Available only if XCP_SEND_EVENT == STD_ON or XCP_SERV_TEXT == STD_ON.
 ...
 * \ingroup     hlp
**********************************************************************************************************************/
XCP_LOCAL_INLINE FUNC(boolean, XCP_CODE) Xcp_Hlp_OccuredPendingTxConfirmation( Xcp_ChannelType XcpChannel );
#endif
 ...
 * \param[in]   entryIdx               The first entry Idx.
 ...
 * \ingroup     hlp
**********************************************************************************************************************/
XCP_LOCAL_INLINE FUNC(uint8, XCP_CODE) Xcp_Hlp_EventDaq( uint8_least XcpChannel, uint16 CoreIdx, P2CONST(Xcp_ChannelStruct, AUTOMATIC, XCP_VAR_NOINIT) pXcp,
                                                         uint8_least Daq, Xcp_OdtEntryIdxType EntryIdx );
 ...
 * \ingroup     hlp
**********************************************************************************************************************/
XCP_LOCAL_INLINE FUNC(uint8, XCP_CODE) Xcp_Hlp_StimEventStatus( uint8_least XcpChannel, uint8_least Daq, uint8 Action );
 ...
 * \ingroup     hlp
**********************************************************************************************************************/
XCP_LOCAL_INLINE FUNC(uint8, XCP_CODE) Xcp_Hlp_EventStim( P2CONST(Xcp_ChannelStruct, AUTOMATIC, XCP_VAR_NOINIT) pXcp, 
                                                          uint8_least Daq,
 ...
 * \reentrant   FALSE
 * \synchronous TRUE
 * \config      Available only if XCP_TL_COMMAND == STD_ON.
 * \pre         -
 * \ingroup     cmdStd
**********************************************************************************************************************/
...
#if( XCP_DAQ_EVENT_INFO == STD_ON ) && ( XCP_MAX_EVENT > 0 )
/**********************************************************************************************************************
                                                         ...
                                                         Xcp_OdtEntryIdxType EntryIdx )
{
  ...
  for( odt = 0; odt < pXcp->DaqList.nOdtsDaq[Daq]; odt++ )
  {
    ...
    pDto[2u] = (uint8)(pXcp->DaqList.FirstOdt[Daq] + odt) | Xcp_OverloadFlag[XCP_CHANNEL_IDX][CoreIdx];
...
# endif
    ...
    for( odtEntry = 0u; odtEntry < pXcp->DaqList.nOdtEntriesOdt[pXcp->DaqList.FirstOdt[Daq] + odt]; odtEntry++ )
    {
                                                          ...
                                                          Xcp_OdtEntryIdxType EntryIdx )
{
  ...
  for( odt = pXcp->DaqList.FirstOdt[Daq];
       odt < (uint8_least)(pXcp->DaqList.FirstOdt[Daq] + pXcp->DaqList.nOdtsDaq[Daq]);
       odt++ )
  {
...
/**********************************************************************************************************************
 *  Xcp_Hlp_OccuredPendingTxConfirmation()
 *********************************************************************************************************************/
/*!
 *
 * Internal comment removed.
...
} /* Xcp_Hlp_CheckPendingForAsynchEvents */
#endif
...
XCP_LOCAL FUNC(void, XCP_CODE) Xcp_CmdStd_Connect( Xcp_ChannelType XcpChannel, CONSTP2CONST(uint8, AUTOMATIC, XCP_APPL_VAR) CmdPtr )
{
  ...
  Xcp_ChannelInfo[XCP_CHANNEL_IDX].SessionStatus = XCP_SESSION_CONNECTED;
...
XCP_LOCAL FUNC(void, XCP_CODE) Xcp_CmdStd_ShortUpload( Xcp_ChannelType XcpChannel, CONSTP2CONST(uint8, AUTOMATIC, XCP_APPL_VAR) CmdPtr )
{
  ...
  /* #20 If the number of requested bytes to be uploaded exceeds max CTO-1:
   *       Respond error XCP_CMD_OUT_OF_RANGE. */
  if( size > XCP_CRM_SHORT_UPLOAD_MAX_SIZE(XCP_CHANNEL_IDX) )
  {
    errorCode = XCP_CMD_OUT_OF_RANGE;
  }
...
#  if ( kXcpChecksumMethod == XCP_CHECKSUM_TYPE_CRC16CCITT ) || \
  ...
  Xcp_ChannelInfo[XCP_CHANNEL_IDX].CheckSum = (tXcpChecksumType)(0xFFFFFFFFu);
#  else
...
XCP_LOCAL FUNC(void, XCP_CODE) Xcp_CmdStd_UserCmd( Xcp_ChannelType XcpChannel, CONSTP2CONST(uint8, AUTOMATIC, XCP_APPL_VAR) CmdPtr )
{
  ...
  Xcp_CmdHlp_PrepareErrorOrPending( XCP_CHANNEL_IDX, XcpAppl_UserService( CmdPtr ) );
...
# if( XCP_BLOCK_DOWNLOAD == STD_ON )
  if( size > XCP_MAX_DOWNLOAD_BYTES )
# else
...
XCP_LOCAL FUNC(void, XCP_CODE) Xcp_CmdCal_ShortDownload( Xcp_ChannelType XcpChannel, CONSTP2CONST(uint8, AUTOMATIC, XCP_APPL_VAR) CmdPtr )
{
  ...
  if( size > XCP_CRM_SHORT_DOWNLOAD_MAX_SIZE(XCP_CHANNEL_IDX) )
  {
    errorCode = XCP_CMD_OUT_OF_RANGE;
  }
#  if( XCP_SEED_KEY == STD_ON )
...
# endif
  ...
  else
  {
    ...
    Xcp_ChannelInfo[XCP_CHANNEL_IDX].ResponseFrame.b[XCP_CRM_START_STOP_FIRST_PID] = Xcp_ChannelInfo[XCP_CHANNEL_IDX].DaqList.FirstOdt[daqNumber];
...
# if( XCP_DAQ_RESUME == STD_ON )
  ...
  /* PRQA S 3109 3 */ /* MD_MSR_14.3 */
  /* PRQA S 0310 2 */ /* MD_Xcp_0310 */
  Xcp_PutVal16( &Xcp_ChannelInfo[XCP_CHANNEL_IDX].ResponseFrame.w[XCP_CRM_GET_DAQ_PROCESSOR_INFO_MAX_DAQ >> 1u], XCP_MAX_DAQ );
...
XCP_LOCAL FUNC(void, XCP_CODE) Xcp_CmdDaq_GetDaqEventInfo( Xcp_ChannelType XcpChannel, CONSTP2CONST(uint8, AUTOMATIC, XCP_APPL_VAR) CmdPtr )
{
  ...
  else
  {
    ...
    Xcp_ChannelInfo[XCP_CHANNEL_IDX].MTA = XcpAppl_GetPointer( XCP_CHANNEL_IDX, 0x00, (const Xcp_AddressPtrType)kXcpEventName[eventNumber] );
...
XCP_LOCAL FUNC(void, XCP_CODE) Xcp_CmdDaq_AllocOdt( Xcp_ChannelType XcpChannel, CONSTP2CONST(uint8, AUTOMATIC, XCP_APPL_VAR) CmdPtr )
{
  ...
  {
    Xcp_ChannelInfo[XCP_CHANNEL_IDX].DaqList.FirstOdt[daqNumber] = (uint8)Xcp_ChannelInfo[XCP_CHANNEL_IDX].DaqList.nOdts;
...
XCP_LOCAL FUNC(void, XCP_CODE) Xcp_CmdDaq_AllocOdtEntry( Xcp_ChannelType XcpChannel, CONSTP2CONST(uint8, AUTOMATIC, XCP_APPL_VAR) CmdPtr )
{
  ...
  else
  {
    ...
    Xcp_ChannelInfo[XCP_CHANNEL_IDX].DaqList.FirstOdtEntry[odtIdx] = (uint8)Xcp_ChannelInfo[XCP_CHANNEL_IDX].DaqList.nOdtEntries;
...
XCP_LOCAL FUNC(void, XCP_CODE) Xcp_CmdPgm_GetSectorInfo( Xcp_ChannelType XcpChannel, CONSTP2CONST(uint8, AUTOMATIC, XCP_APPL_VAR) CmdPtr )
{
  ...
  else
  {
    ...
    {
      Xcp_ChannelInfo[XCP_CHANNEL_IDX].ResponseFrameLength = 8;
...
FUNC(void, XCP_CODE) Xcp_Init( void )
{
...
#endif
  {
    ...
    Xcp_Rtm_Start(Xcp_Init); /* PRQA S 3109 */ /* MD_MSR_14.3 */
...
# endif
    ...
    Xcp_Rtm_Stop(Xcp_Init); /* PRQA S 3109 */ /* MD_MSR_14.3 */
  }
#if( XCP_CONTROL == STD_ON )
  else
  {
...
# endif
...
#endif
    {
      ...
      Xcp_ChannelInfo[XCP_CHANNEL_IDX].SessionStatus &= (uint16)(~(uint16)XCP_SESSION_POLLING); /* PRQA S 0277 */ /* MD_Xcp_0277 */
...
# endif
    ...
    {
      /* ----- Implementation ----------------------------------------------- */
      /* #30 Set channel or TL information. */
...
# endif
...
#endif
    {
      ...
      activeTl = Xcp_ChannelInfo[XcpChannel].ActiveTl;
...
# endif
...
#endif
    {
      ...
      Xcp_Rtm_Start(Xcp_TlRxIndication); /* PRQA S 3109 */ /* MD_MSR_14.3 */
...
# endif
      ...
      Xcp_Rtm_Stop(Xcp_TlRxIndication); /* PRQA S 3109 */ /* MD_MSR_14.3 */
...
# endif
...
#endif
    {
      /* ----- Implementation ----------------------------------------------- */
      /* #30 Start runtime measurement. */
      Xcp_Rtm_Start(Xcp_TlTxConfirmation); /* PRQA S 3109 */ /* MD_MSR_14.3 */
      ...
      Xcp_Rtm_Stop(Xcp_TlTxConfirmation); /* PRQA S 3109 */ /* MD_MSR_14.3 */
...
#  if (XCP_DEV_ERROR_DETECT == STD_ON)
    ...
    else if( EventChannel >= XCP_MAX_EVENT )
    {
...
#  if( XCP_NUMBER_OF_CORES > 1u )
    ...
    {
      Xcp_BswErrorId = XCP_E_INVALID_CORE_ID;
    }
#  endif
    else
# endif
    {
      /* ----- Implementation ----------------------------------------------- */
      /* #30 Start runtime measurement. */
      Xcp_Rtm_Start(Xcp_Event); /* PRQA S 3109 */ /* MD_MSR_14.3 */
        ...
        { /* #50 Check if current DAQ list is active and assigned to this event channel. */
            ...
            )
          {
            ...
            if( pXcp->DaqList.DaqPrescalerCnt[daq] == 0u )
            {
              ...
              entryIdx = pXcp->DaqList.FirstOdtEntry[pXcp->DaqList.FirstOdt[daq]];
...
# if( XCP_STIM == STD_ON )
              ...
              {
                retVal |= Xcp_Hlp_EventStim(pXcp, daq, entryIdx);
              }
              else
# endif
              /* #80 Otherwise call EventDaq to assemble DAQ list. */
              {
                retVal |= Xcp_Hlp_EventDaq(XCP_CHANNEL_IDX, coreIdx, pXcp, daq, entryIdx);
              }
            }
          }
        }
      ...
      Xcp_Rtm_Stop(Xcp_Event); /* PRQA S 3109 */ /* MD_MSR_14.3 */
    }
...
# endif
    {
      eventFrameIsFree = Xcp_Hlp_OccuredPendingTxConfirmation( XCP_CHANNEL_IDX );
...
# endif
    {
      /* #30 Return pointer to complete XCP data (required for falsh programming with a flash kernel). */
...
FUNC(void, XCP_CODE) Xcp_MainFunction( void )
{
...
#endif
  {
    /* ----- Implementation ----------------------------------------------- */
    /* #20 Start runtime measurement. */
    Xcp_Rtm_Start(Xcp_MainFunction); /* PRQA S 3109 */ /* MD_MSR_14.3 */
...
# endif
    ...
    Xcp_Rtm_Stop(Xcp_MainFunction); /* PRQA S 3109 */ /* MD_MSR_14.3 */
...
# if( XCP_DEV_ERROR_REPORT == STD_ON )
    ...
    {
      (void)Det_ReportError( XCP_MODULE_ID, XCP_INSTANCE_ID_DET, XCP_SID_MAINFUNCTION, Xcp_BswErrorId );
    }
# endif
#endif
  }
...
/* Justification for module-specific MISRA deviations:
  ...
  MD_Xcp_0277: rule 3.1
      Reason:     Integer conversion from an unsinged type.
      Risk:       There is no risk as the base type is unsigned which is converted to signed and back to unsigned again.
  ...
  MD_Xcp_0342: rule: 19.13
     Reason:      K&R compilers do not support the ISO glue operator ##. Identifier has to be a combination of both IDs.
     Risk:        Compile error.
     Prevention:  Not required.

  MD_Xcp_0488: rule: 17.4
      Reason:     Determine size of structure.
      Risk:       None.
      Prevention: Covered by code review.
