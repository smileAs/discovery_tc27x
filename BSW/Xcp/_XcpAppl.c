...
unsigned int GetBaseAddr(void)
{
  ...
  return (unsigned int)(moduleInfo.lpBaseOfDll);
}
# undef CONST
                                                       ...
                                                       const Xcp_AddressPtrType Addr )
{
  ...
  XCP_DUMMY_STATEMENT( AddrExt ); /* PRQA S 3112, 3199 */ /* MD_MSR_14.2 */ /*lint -e{438} */
...
FUNC(uint8, XCP_CODE) XcpAppl_CopyCalPage( uint8 SrcSeg, uint8 SrcPage, uint8 DestSeg, uint8 DestPage )
{
  ...
  /* #20 If all parameters are valid, copy a page from SrcSeg:SrcPage to DestSeg:DestPage. */
                                            ...
                                            uint8 Size )
{
  ...
  /* If Programming was not started, return XCP_ERR_SEQUENCE. */
  if( XcpAppl_ProgramStartFlag == 0 )
  {
    retVal = XCP_ERR_SEQUENCE;
  }
  /* Otherwise, program a flash area specified by address and size with data. */
  else
  {
    /* to be implemented */
  }
                                          ...
                                          uint8 Id )
{
  ...
  { /* Example code, fit to your needs: */
    case( XCP_IDT_ASCII ): 
      /* Handle ASCII text. */
      *Data = (P2VAR(uint8, AUTOMATIC, XCP_VAR_NOINIT))&XcpAppl_AsciiString;
      retVal = (uint32)sizeof(XcpAppl_AsciiString)-1;
      break;
    case( XCP_IDT_VECTOR_MAPNAMES ): 
      /* Handle map file name. */
      *Data = (P2VAR(uint8, AUTOMATIC, XCP_VAR_NOINIT))&XcpAppl_MapName;
                                             ...
                                             boolean MeasurementStart )
{
  ...
  XcpAppl_MemCpy(&(XcpAppl_ResumeNVMemory[XcpChannel][0]), (uint8*)Channel, size);
