...
#define E2EXF_DUMMY_STATEMENT(x)                        (x)=(x) /* PRQA S 3453 */ /* MD_MSR_19.7 */
  ...
  uint16 minInputBufferLength)
{
  ...
  if (options->inPlace == TRUE)
  {
    ...
    else if ((options->executeDespiteDataUnavailability == FALSE) && (buffer == NULL_PTR) && (inputBufferLength == 0U))
    {
      retVal = E_SAFETY_HARD_RUNTIMEERROR;
    }
    else
  ...
  else /* out-of-place */
  {
    ...
    /* execution of E2EXf is explicitly not wanted/expected in this case */
    else if ((options->executeDespiteDataUnavailability == FALSE) && (inputBuffer == NULL_PTR) && (inputBufferLength == 0U))
    {
      retVal = E_SAFETY_HARD_RUNTIMEERROR;
    }
    else
...
# endif
  {
    /* ----- Implementation ----------------------------------------------- */
    /* [SWS_E2EXf_00133] */
    if (E2EXf_ModuleInitialized == (uint8)E2EXF_INIT)
    {
    ...
    if (E2EXf_ModuleInitialized == (uint8)E2EXF_INIT)
    {
      ...
      if (retVal == E_OK)
      {
        ...
        else
        {
          ...
          if ((pCheckBuffer == NULL_PTR) && (inputBufferLength == 0U))
          {
