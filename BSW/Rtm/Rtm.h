...
/* API ID */
# define RTM_API_ID_CPULOADMEASUREMENT     ((uint8)0x01u)
# define RTM_API_ID_GETVERSIONINFO         ((uint8)0x02u)
# define RTM_API_ID_INIT                   ((uint8)0x03u)
# define RTM_API_ID_INITMEMORY             ((uint8)0x04u)
# define RTM_API_ID_MAINFUNCTION           ((uint8)0x05u)
# define RTM_API_ID_STARTMEASUREMENT       ((uint8)0x06u)
# define RTM_API_ID_STOPMEASUREMENT        ((uint8)0x07u) 
# define RTM_API_ID_GETMEASUREMENTITEM     ((uint8)0x08u) /* PRQA S 0857 */ /* MD_MSR_1.1_857 */
# define RTM_API_ID_ENTERTASKFCT           ((uint8)0x09u)
# define RTM_API_ID_LEAVETASKFCT           ((uint8)0x0Au)
# define RTM_API_ID_ENTERISRFCT            ((uint8)0x0Bu)
# define RTM_API_ID_LEAVEISRFCT            ((uint8)0x0Cu)
...
#if (RTM_DEV_ERROR_DETECT == STD_ON)
# define Rtm_ReportDetError(api, err)                              (void)Det_ReportError( RTM_MODULE_ID, 0, (api), (err)) /* PRQA S 3453 */ /* MD_MSR_19.7 */

/* ESCAN00076463, ESCAN00076464 */
# define Rtm_CheckDetErrorReturnVoid( condition, api, err )        {                                                           \
                                                                     if(!(condition))                                          \
                                                                     {                                                         \
                                                                       (void)Det_ReportError( RTM_MODULE_ID, 0, (api), (err)); \
                                                                       return;                                                 \
                                                                     }                                                         \
                                                                   } /* PRQA S 3458 */ /* MD_MSR_19.4 */

# define Rtm_CheckDetErrorReturnValue(condition, api, err, ret)    {                                                           \
                                                                     if(!(condition))                                          \
                                                                     {                                                         \
                                                                       (void)Det_ReportError( RTM_MODULE_ID, 0, (api), (err)); \
                                                                       return(ret);                                            \
...
# define Rtm_CheckDetErrorContinue( condition, api, err )          {                                                           \
                                                                     if(!(condition))                                          \
                                                                     {                                                         \
                                                                       (void)Det_ReportError( RTM_MODULE_ID, 0, (api), (err)); \
                                                                     }                                                         \
...
#else /* (RTM_DEV_ERROR_DETECT == STD_ON) */
# define Rtm_CheckDetErrorReturnVoid( condition, api, err )
# define Rtm_CheckDetErrorReturnValue( condition, api, err, ret )
# define Rtm_CheckDetErrorContinue( condition, api, err )
# define Rtm_ReportDetError(api, err)
...
extern VAR(Rtm_MeasurementTimestampType, RTM_VAR_NOINIT) Rtm_PrevTimerValue;
...
extern VAR(uint8,  RTM_VAR_NOINIT)                  Rtm_TimerOverrunCnt;
#  endif
 ...
 *  \details     It must be called cyclically with a high frequence than the used timer.
 *  \context     TASK|ISR2
 *  \reentrant   FALSE
 *  \synchronous TRUE
 ...
 *  \details     Returns average CPU load, current CPU load, min .
