...
))
{
  ...
  *(uint32*)PCXIAddr = Current->PCXI;                                                                                   /* SBSW_OS_HAL_PWA_PCXI_SERVICEFUNCTION */ /* PRQA S 0306 */ /* MD_Os_Hal_0306 */
  
  /* #30 Modify stack information in the context, because it is initialized with 0 */
  if(Os_MpIsEnabled() != 0)
  {
    Next->MpuRegionForStackLow = Os_Hal_Mfcr(OS_HAL_COREMPU_DPR_LOW(0));                                                /* SBSW_OS_HAL_PWA_CALLER */
    Next->MpuRegionForStackUpper = Os_Hal_Mfcr(OS_HAL_COREMPU_DPR_UPPER(0));                                            /* SBSW_OS_HAL_PWA_CALLER */
  }
  PCXIAddr = Os_Hal_GetCsaAddress(Next->PCXI);
  *(uint32*)(PCXIAddr + OS_HAL_CSA_SP_OFFSET) = Os_Hal_Mfa10();                                                         /* SBSW_OS_HAL_PWA_PCXI_SERVICEFUNCTION */ /* PRQA S 0306 */ /* MD_Os_Hal_0306 */
