...
#define OS_BARRIER_COUNTERTHRESHOLD     (((Os_BarrierCounterType)1 << ((sizeof(Os_BarrierCounterType) * 8) - 1)))
...
))
{
  ...
  return (Os_StdReturnType)((Os_BarrierCounterType)(Value - Compare) >= OS_BARRIER_COUNTERTHRESHOLD);
}
...
))
{
  ...
  if( Barrier->CheckOsStart == TRUE )                                                                                   /* COV_OS_UNSUPPORTED93797 */
  {
    ...
    if(Os_CoreOsIsStarted(Os_TaskGetCore(Barrier->TaskPtr)) != 0)                                                       /* SBSW_OS_BR_COREOSISSTARTED_001 */ /* SBSW_OS_BR_TASKGETCORE_001 */
    {
      /* #30 If the addressed attendee is detached or has already reached the counter value, the result
       * is !0, otherwise 0. */
      synchronized = (Os_BarrierIsCounterGe(attendeeCounter, ownCounter) != 0)
...
{
  return Os_ErrExtendedCheck((Os_StdReturnType)(TaskPtr == CurrentTask));
}
 ...
 *
 */
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE                                                                            /* COV_OS_UNSUPPORTED93797 *//* PRQA S 3219 */ /* MD_Os_3219 */
...
))                                                                                                                      /* COV_OS_UNSUPPORTED93797 */
{
  ...
  return barrierRefs[BarrierId];                                                                                        /* PRQA S 3689 */ /* MD_Os_3689 */
}
...
)
{
  ...
  do
  {
    ...
    for(attendeeIdx = 0; attendeeIdx < Barrier->AttendeeCount; ++attendeeIdx)
    {
      synchronized = (Os_BarrierIsSynchronized(Barrier, attendeeIdx)) && synchronized;                                  /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */ /* SBSW_OS_FC_PRECONDITION */
    }
...
)                                                                                                                       /* COV_OS_UNSUPPORTED93797 */
{
  ...
  else
  {
    ...
    if(OS_UNLIKELY(Os_ErrCheckPointerIsNotNull(barrier) == OS_CHECK_FAILED))                                            /* SBSW_OS_BR_ERRCHECKPOINTERISNOTNULL_001 */
    {
...
\ID SBSW_OS_BR_COREASRGERTBARRIERS_001
 \DESCRIPTION    Os_CoreAsrGetBarriers is called with the return values of Os_Core2AsrCore and Os_CoreCheckIsLocalCore.
...
\ID SBSW_OS_BR_TASKGETCORE_001
 \DESCRIPTION    Os_TaskGetCore is called with a pointer derived from a given argument.
                 For OS internal calls it is ensured that this code is never called. (CM_OS_BARRIER_CHECKOSSTART_M)
                 For calls via external user API the validity is ensured by runtime check. (CM_OS_BARRIER_TASKPTR_R)
