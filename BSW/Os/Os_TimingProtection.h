...
#if !defined (OS_TIMINGPROTECTION_H)                                                                                    /* PRQA S 0883 */ /* MD_Os_0883 */
...
{
  Os_Assert((Os_StdReturnType)(TpThread->ThreadKind == OS_TPTHREADKIND_OWNER));
  return (P2CONST(Os_TpOwnerThreadConfigType, AUTOMATIC, OS_CONST))(TpThread);                                          /* PRQA S 0310, 3305 */ /* MD_Os_0310, MD_Os_3305 */
}
...
))
{
  ...
  if(Os_TpIsEnabled() != 0)
  {
    FUNC(void, OS_CODE) Os_TpThreadInitInternal                                                                         /* PRQA S 3221 */ /* MD_Os_3221 */
    (
      P2CONST(Os_TpConfigType, AUTOMATIC, OS_CONST) Config,
      P2CONST(Os_TpThreadConfigType, AUTOMATIC, OS_CONST) Current
    );
...
))
{
  ...
  if(Os_TpIsEnabled() != 0)
  {
    FUNC(void, OS_CODE) Os_TpThreadKillInternal                                                                         /* PRQA S 3221 */ /* MD_Os_3221 */
    (
      P2CONST(Os_TpThreadConfigType, AUTOMATIC, OS_CONST) Thread
    );
...
))
{
  ...
  if(Os_CounterIsFutureValue(Os_TimerHrtGetCounter(Config->Timer), Deadline, Now) != 0)                                 /* SBSW_OS_TP_COUNTERISFUTUREVALUE_001 */ /* SBSW_OS_TP_TIMERHRTGETCOUNTER_001 */
  {
...
))
{
  ...
  return Os_TimerSub(maxAllowedValue, maxCountingValue, Value, Sub);
}
 ...
 *
 */
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE Os_InterArrivalTimeType, OS_CODE,                                          /* PRQA S 3219 */ /* MD_Os_3219 */
...
))
{
  if(Os_TpIsEnabled() != 0)
  {
    ...
    if(Next->ThreadKind == OS_TPTHREADKIND_OWNER)
    {
      ...
      Os_Assert((Os_StdReturnType)(Current->ThreadKind == OS_TPTHREADKIND_OWNER));
...
))
{
  if(Os_TpIsEnabled() != 0)
  {
    /* #10 If the current thread's budgets are inherited: */
    if(Current->ThreadKind == OS_TPTHREADKIND_HEIR)
    {
    ...
    {
      /* #40 If the next thread is a timing protection owner: */
      if(Next->ThreadKind == OS_TPTHREADKIND_OWNER)
      {
        P2CONST(Os_TpOwnerThreadConfigType, AUTOMATIC, OS_CONST) currentOwner;
        P2CONST(Os_TpOwnerThreadConfigType, AUTOMATIC, OS_CONST) nextOwner;

        /* Optimization: If the current thread equals the next thread, there is no need to switch.
         * As this is the UNLIKELY case, the switch from current to next is performed always, without any condition. */
        ...
        if(currentOwner->Dyn->MonitoredBudgets != NULL_PTR)
        {
          ...
          Os_Assert((Os_StdReturnType)(currentOwner->Dyn->MonitoredBudgets == &currentOwner->ExecutionBudget));
...
))
{
  ...
  if(Os_TpMonitorPriority[Violation] > Os_TpMonitorPriority[Current->Dyn->Violation])                                   /* PRQA S 3689 */ /* MD_Os_3689 */
  {
...
))
{
  if(Os_TpIsEnabled() != 0)
  {
    ...
    Os_Assert((Os_StdReturnType)(currentOwner->Dyn->DelayLevel != 0));
  }
}
...
))
{
  if(Os_TpIsEnabled() != 0)
  {
    ...
    if (currentOwner->Dyn->DelayLevel == 0)
    {
...
{
  return (Os_StdReturnType)(OS_CFG_TIMING_PROTECTION == STD_ON);
}
