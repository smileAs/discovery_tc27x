 ...
 *  \details        Sets all dynamic data of the given task to its configured startup settings, except
 *                  ActivationsAllowed.
 *
 ...
 *  \pre            ActivationsAllowed has been initialized before.
 **********************************************************************************************************************/
...
( void ))
{
  return (Os_StdReturnType)(OS_CFG_PRETASKHOOK == STD_ON);
}
...
{
  return Os_ErrExtendedCheck((Os_StdReturnType)(Task->Reschedule == OS_TASKSCHEDULE_ALLOWED));
}
...
{
  /* #10 Reset all task attributes and all its children, except ActivationsAllowed. */
...
)
{
  ...
  Os_Assert((Os_StdReturnType)(Task != NULL_PTR));
  Os_Assert((Os_StdReturnType)(Os_TaskGetThread(Task) != NULL_PTR));                                                    /* SBSW_OS_FC_PRECONDITION */
  Os_Assert(Os_ErrIsValueLo((uint32)(Os_TaskGetId(Task)), (uint32)(OS_TASKID_COUNT)));                                  /* SBSW_OS_FC_PRECONDITION */ /* PRQA S 3355 */ /* MD_Os_3355 */
  ...
  Os_Assert((Os_StdReturnType)(Os_TaskGetMaxActivations(Task) > 0 ));                                                   /* SBSW_OS_FC_PRECONDITION */ /* PRQA S 0505 */ /* MD_Os_0505 */
  Os_Assert((Os_StdReturnType)(Os_TaskGetDyn(Task) != NULL_PTR));                                                       /* SBSW_OS_FC_PRECONDITION */
 ...
 *
 */
FUNC(Os_StatusType, OS_CODE) Os_Api_ActivateTask                                                                        /* PRQA S 1503 */ /* MD_MSR_14.1 */
(
  TaskType TaskID
)
{
  ...
  else
  {
    ...
    else
    {
      ...
      if(OS_LIKELY(localCore == targetCore))                                                                            /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */
      {
        ...
        {
          if(Os_ThreadIsTask(currentThread) != 0)                                                                       /* SBSW_OS_TASK_THREADISTASK_001 */
          {
            ...
            if((Os_SchedulerTaskSwitchIsNeeded(scheduler) != 0))                                                        /* SBSW_OS_TASK_SCHEDULERTASKSWITCHISNEEDED_001 */
            {
 ...
 *
 */
FUNC(Os_StatusType, OS_CODE) Os_TaskActivateLocalTask                                                                   /* PRQA S 1503 */ /* MD_MSR_14.1 */
(
  P2CONST(Os_TaskConfigType, AUTOMATIC, OS_CONST) Task
)
{
  ...
  /* #30 Check whether inter-arrival time is okay. */
  else if(OS_UNLIKELY(Os_TpCheckInterArrival(timingProtection, Os_ThreadGetTpConfig(taskThread)) == OS_CHECK_FAILED))   /* SBSW_OS_TASK_TPCHECKINTERARRIVAL_001 */ /* SBSW_OS_TASK_THREADGETTPCONFIG_001 */
  {
    status = OS_STATUS_OK;
  }
  else
...
)
{
  ...
  Os_Assert((Os_StdReturnType)((Os_TaskGetDyn(Task)->ActivationsAllowed) > 0));                                         /* SBSW_OS_FC_PRECONDITION */

  timingProtection = Os_CoreAsrGetTimingProtection(Os_ThreadGetCore(taskThread));                                       /* SBSW_OS_TASK_COREASRGETTIMINGPROTECTION_001 */ /* SBSW_OS_TASK_THREADGETCORE_002 */

  /* #10 Check whether inter-arrival time is okay. */
  if(OS_UNLIKELY(Os_TpCheckInterArrival(timingProtection, Os_ThreadGetTpConfig(taskThread)) == OS_CHECK_FAILED))        /* COV_OS_NOFUNCCALL *//* SBSW_OS_TASK_TPCHECKINTERARRIVAL_001 */ /* SBSW_OS_TASK_THREADGETTPCONFIG_001 */
  {
    status = OS_STATUS_OK;
  }
  else
 ...
 *
 */
FUNC(Os_StatusType, OS_CODE) Os_Api_TerminateTask(void)                                                                 /* PRQA S 1503 */ /* MD_MSR_14.1 */
{
  Os_StatusType status;
  P2CONST(Os_ThreadConfigType, AUTOMATIC, OS_CONST) currentThread;

  currentThread = Os_CoreGetThread();                                                                                   /* PRQA S 1527 */ /* MD_Os_1527 */
  ...
  else if(OS_UNLIKELY(Os_ThreadCheckSpinlocksReleased(currentThread) == OS_CHECK_FAILED))                               /* COV_OS_HALPLATFORMTESTEDASMULTICORE */ /* SBSW_OS_TASK_THREADCHECKSPINLOCKSRELEASED_001 */
  {
 ...
 *
 */
FUNC(Os_StatusType, OS_CODE) Os_Api_ChainTask                                                                           /* PRQA S 1503 */ /* MD_MSR_14.1 */
(
  TaskType TaskID
)
{
  ...
  else
  {
    ...
    else
    {
      ...
      if(currentTask == targetTask)
      {
        ...
        if(OS_UNLIKELY(Os_TpCheckInterArrival(timingProtection, Os_ThreadGetTpConfig(currentThread)                     /* SBSW_OS_TASK_TPCHECKINTERARRIVAL_001 */ /* SBSW_OS_TASK_THREADGETTPCONFIG_001 */
                                              ) == OS_CHECK_FAILED))
        {
      ...
      {
        P2CONST(Os_CoreAsrConfigType, AUTOMATIC, OS_CONST) targetCore;
        targetCore = Os_ThreadGetCore(Os_TaskGetThread(targetTask));                                                    /* SBSW_OS_TASK_THREADGETCORE_002 */ /* SBSW_OS_TASK_TASKGETTHREAD_001 */
 ...
 *
 */
FUNC(Os_StatusType, OS_CODE) Os_Api_Schedule(void)                                                                      /* PRQA S 1503 */ /* MD_MSR_14.1 */
{
  Os_StatusType status;
  P2CONST(Os_ThreadConfigType, AUTOMATIC, OS_CONST) currentThread;
  P2CONST(Os_TaskConfigType, AUTOMATIC, OS_CONST) currentTask;
  ...
  else if(OS_UNLIKELY(Os_TaskCheckIsScheduleAllowed(currentTask) == OS_CHECK_FAILED))                                   /* COV_OS_UNSUPPORTED88879 *//* SBSW_OS_TASK_TASKCHECKISSCHEDULEALLOWED_001 */
  {
  ...
  else
  {
    ...
    if(Os_SchedulerReleaseRunningPrio(localScheduler) != 0)                                                             /* SBSW_OS_TASK_SCHEDULERRELEASERUNNINGPRIO_001 */
    {
 ...
 *
 */
FUNC(void, OS_CODE) Os_TaskInit                                                                                         /* PRQA S 1505 */ /* MD_MSR_8.10 */
(
  P2CONST(Os_TaskConfigType, AUTOMATIC, OS_CONST) Task,
  AppModeType AppMode
)
{
  ...
  if(Task != Os_CoreGetIdleTask(Os_ThreadGetCore(taskThread)))                                                          /* SBSW_OS_TASK_COREGETIDLETASK_001 */ /* SBSW_OS_TASK_THREADGETCORE_001 */
  {
    ...
    if((Task->AutostartModes & AppMode) != 0)
    {
 ...
 *
 */
FUNC(void, OS_CODE) Os_TaskKillAll                                                                                      /* PRQA S 1503 */ /* MD_MSR_14.1 */
(
  P2CONST(Os_TaskConfigType, AUTOMATIC, OS_CONST) Task
)
{
  ...
  Os_Assert((uint32)(Os_ThreadGetCore(Os_CoreGetThread()) == Os_ThreadGetCore(thread)));                                /* SBSW_OS_TASK_THREADGETCORE_002 */ /* SBSW_OS_TASK_THREADGETCORE_001 */ /* PRQA S 1527 */ /* MD_Os_1527 */

  /* #20 Assert that the given task is not the idle loop or any other OS task. */
  Os_Assert((uint32)(Os_TaskGetId(Task) != INVALID_TASK));                                                              /* SBSW_OS_FC_PRECONDITION */

  /* #30 Tell the scheduler to remove all instances of the given task. */
  ...
  /* #40 Inform the trace object about the killing. */
  ...
  /* #50 Initialize the given task, so it can be activated again. */
...
)
{
  ...
  localCore = Os_ThreadGetCore(Os_CoreGetThread());                                                                     /* SBSW_OS_TASK_THREADGETCORE_001 */
  targetCore = Os_ThreadGetCore(Os_TaskGetThread(Task));                                                                /* SBSW_OS_TASK_THREADGETCORE_002 */ /* SBSW_OS_FC_PRECONDITION */
  ...
  else
  {
    ...
    status = Os_XSigSend_GetTaskState(targetCore, Os_CoreGetThread(), Task->TaskId, State);                             /* SBSW_OS_TASK_XSIGSEND_001 */
  }
 ...
 *
 */
FUNC(void, OS_CODE) Os_TaskKill(void)                                                                                   /* PRQA S 1503 */ /* MD_MSR_14.1 */
{
  P2CONST(Os_ThreadConfigType, AUTOMATIC, OS_CONST) currentThread;
  P2CONST(Os_TaskConfigType, AUTOMATIC, OS_CONST) targetTask;
  ...
  Os_Assert((uint32)(Os_CoreGetIdleTask(core) != targetTask));                                                          /* SBSW_OS_TASK_COREGETIDLETASK_001 */

  /* One more activation is allowed */
  ++(Os_TaskGetDyn(targetTask)->ActivationsAllowed);                                                                    /* SBSW_OS_TASK_TASKGETDYN_001 */ /* SBSW_OS_TASK_TASKGETDYN_002 */

  /* #20 Tell the scheduler to remove the given task. */
  ...
  /* #40 Reset the given task, so it can be activated again. */
 ...
 *
 */
FUNC(Os_StatusType, OS_CODE) Os_Api_GetTaskID(TaskRefType TaskID)                                                       /* PRQA S 1503 */ /* MD_MSR_14.1 */
{
  Os_StatusType status;
  P2CONST(Os_ThreadConfigType, AUTOMATIC, OS_CONST) currentThread;
 ...
 *
 */
FUNC(Os_StatusType, OS_CODE) Os_Api_GetTaskState                                                                        /* PRQA S 1503 */ /* MD_MSR_14.1 */
(
  TaskType TaskID,
  TaskStateRefType State
)
...
FUNC(void, OS_CODE) Os_TaskCallPreTaskHook(void)
{
  ...
  if(Os_TaskIsPreTaskHookEnabled() != 0)
  {
    P2CONST(Os_ThreadConfigType, AUTOMATIC, OS_CONST) currentThread;
...
\ID SBSW_OS_TASK_TASKGETTHREAD_003
 \DESCRIPTION    Os_TaskGetThread is called with the return value of Os_SchedulerRemoveCurrentTask.
 \COUNTERMEASURE \R [CM_OS_SCHEDULERREMOVECURRENTTASK_R]
