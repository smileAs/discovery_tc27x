...
))
{
  ...
  return XSignal->SendPorts[SendPortIdx];                                                                               /* PRQA S 3689 */ /* MD_Os_3689_Precondition */
}
...
{
  Os_Assert((Os_StdReturnType)(IsrIdx < Os_XSigGetRecvIsrCount(XSignal)));                                              /* SBSW_OS_FC_PRECONDITION */
...
))                                                                                                                      /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */
{
  ...
  if(OS_UNLIKELY(Os_ErrIsValueLo((uint32)FunctionIdx, (uint32)OS_XSIGFUNCTIONIDX_USEDCOUNT) != 0))
  {
    result = (Os_StdReturnType)(XSigIsr->FunctionTable->FunctionDescription[FunctionIdx].FunctionRef != NULL_PTR);      /* PRQA S 3689 */ /* MD_Os_3689_Precondition */
  }
...
))                                                                                                                      /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */
{
  ...
  return OsCfg_XSig_ServiceMap[FunctionIdx];                                                                            /* PRQA S 3689 */ /* MD_Os_3689_Precondition */
}
...
))                                                                                                                      /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */
{
  ...
  while(Os_XSigChannelIsResultAvailable(SendChannel) != 0)                                                              /* SBSW_OS_FC_PRECONDITION */
  {
    ...
    if(OS_LIKELY(comData->SendData->Acknowledge == OS_XSIGSTATE_ASYNC_NACK))
    {
 ...
 *
 * */
        for(idx = (Os_ErrorParmIndexType)0; idx < OS_ERRORPARAMINDEX_COUNT; ++idx)
        {
 ...
 *
 * */
        Os_ErrApplicationError(serviceId, status, &errorParameters);                                                    /* PRQA S 3354 */ /* MD_Os_3354_Param */ /* SBSW_OS_FC_POINTER2LOCAL */
      }

      /* #50 Mark the buffer element as unused. */
      comData->SendData->Acknowledge = OS_XSIGSTATE_ACK;                                                                /* SBSW_OS_XSIG_XSIGCHANNELGETRESULT_001 */
    }
    /* OS_XSIGSTATE_SYNC_NACK are handled by the sender thread, OS_XSIGSTATE_ACK don't require any action. */

    /* #60 Remove the element from the buffer. */
...
{
  return (P2CONST(Os_XSigIsrConfigType, AUTOMATIC, OS_CONST))Thread;                                                    /* PRQA S 0310 */ /* MD_Os_0310 */
}
...
ISR(Os_XSigRecvIsrHandler)                                                                                              /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */
{
  ...
  do
  {
    ...
    for(it = 0; it < xSigIsr->RecvPortCount; ++it)
    {
      ...
      if(Os_CoreIsStartedAsAsr(Os_CoreAsr2Core(sendCore)) != 0)                                                         /* SBSW_OS_XSIG_COREISSTARTEDASASR */ /* SBSW_OS_XSIG_COREASR2CORE_001 */
      {
        /* #50 If the sender's channel index is valid: */
        if(Os_XSigChannelReceiveCheckInRange(&(recvPort->RecvChannel)) != 0)                                            /* SBSW_OS_XSIG_XSIGCHANNELRECEIVECHECKINRANGE */
        {
          /* #60 If the core has send a request: */
          if(Os_XSigChannelIsReceiveElementAvailable(&(recvPort->RecvChannel)) != 0)                                    /* SBSW_OS_XSIG_XSIGCHANNELISRECEIVEELEMENTAVAILABLE_001 */
          {
            P2CONST(Os_XSigComDataConfigType, AUTOMATIC, OS_CONST) comData;
            ...
            /* #70 Check that the function is available. */
            ...
            /* #80 Call the respective function. */
            else
            {
              ...
              inParameter = ((P2CONST(Os_XSigParamsType, AUTOMATIC, OS_VAR_NOINIT))&(sendData->ParameterData));         /* PRQA S 0312 */ /* MD_Os_0312_Volatile */
              outParameter = ((P2VAR(Os_XSigOutputParamsType, AUTOMATIC, OS_VAR_NOINIT))                                /* PRQA S 0312 */ /* MD_Os_0312_Volatile */
                  &(comData->RecvData->ReturnData.OutputParameter));

              functionRef = xSigIsr->FunctionTable->FunctionDescription[sendData->FunctionIdx].FunctionRef;             /* PRQA S 3689 */ /* MD_Os_3689_Check */
    ...
    /* #100 While any buffer had entries. */
  } while(messageReceived != 0);

  /* #110 Resume interrupts. */
  Os_IntResume(&interruptState);                                                                                        /* SBSW_OS_FC_POINTER2LOCAL */
}                                                                                                                       /* PRQA S 6050 */ /* PRQA S 6080 */ /* MD_Os_STCAL */ /* MD_Os_STMIF */
...
#  pragma warning 549   /* This suppresses the warning with regard to "condition always false" by checking against
                           OS_COREASRCOREIDX_COUNT. This can be suppressed, since this warning is only valid for
                           configurations with no XSignal channels. */
# endif
/***********************************************************************************************************************
 *  Os_XSigInit()
 **********************************************************************************************************************/
...
)
{
  ...
  if(Os_XSigIsEnabled() != 0)
  {
    ...
    for(itSendPort = (Os_CoreAsrCoreIdx)0; itSendPort < OS_COREASRCOREIDX_COUNT; ++itSendPort)
    {
 ...
 * */
    ...
    else
    {
      ...
      if(status == OS_STATUS_OK)
      {
        ...
        while(Os_XSigChannelIsRequestHandled(&(sendPort->SendChannel), comData) == 0)                                   /* SBSW_OS_XSIG_XSIGCHANNELISREQUESTHANDLED_001 */
        {
          /*! Internal comment removed.
 *
 *
...
(
  ...
  P2CONST(Os_XSigParamsType, AUTOMATIC, OS_VAR_NOINIT) InParameter,
  P2VAR(Os_XSigOutputParamsType, AUTOMATIC, OS_VAR_NOINIT) OutParameter                                                 /* PRQA S 3673 */ /* MD_Os_3673_FunctionPointer */
)
{
  Os_StatusType status;
  TaskType taskId;

  OS_IGNORE_UNREF_PARAM(OutParameter);                                                                                  /* PRQA S 3112 */ /* MD_Os_3112 */
  ...
  else
  {
    ...
    if(OS_UNLIKELY(Os_CoreCheckIsLocalCore(taskCore, localCore) == OS_CHECK_FAILED))                                    /* COV_OS_XSIGDOUBLECHECK *//* SBSW_OS_XSIG_CORECHECKISLOCALCORE_001 */
    {
      status = OS_STATUS_CORE_CROSS_DATA;
    }
    else if( OS_UNLIKELY(Os_AppCheckAccess(SendApp, permittedApplications) == OS_CHECK_FAILED) )                        /* COV_OS_XSIGDOUBLECHECK *//* SBSW_OS_FC_PRECONDITION */
    {
      status = OS_STATUS_CORE_CROSS_DATA;
    }
    else
...
)
{
  ...
  else
  {
    ...
    if(OS_UNLIKELY(Os_CoreCheckIsLocalCore(counterCore, localCore) == OS_CHECK_FAILED))                                 /* COV_OS_XSIGDOUBLECHECK *//* SBSW_OS_XSIG_CORECHECKISLOCALCORE_002 */
    {
      status = OS_STATUS_CORE_CROSS_DATA;
    }
    else if( OS_UNLIKELY(Os_AppCheckAccess(SendApp, permittedApplications) == OS_CHECK_FAILED) )                        /* COV_OS_XSIGDOUBLECHECK *//* SBSW_OS_FC_PRECONDITION */
...
)
{
  ...
  else
  {
    ...
    if(OS_UNLIKELY(Os_CoreCheckIsLocalCore(alarmCore, localCore) == OS_CHECK_FAILED))                                   /* COV_OS_XSIGDOUBLECHECK *//* SBSW_OS_XSIG_CORECHECKISLOCALCORE_003 */
    {
      status = OS_STATUS_CORE_CROSS_DATA;
    }
    else if( OS_UNLIKELY(Os_AppCheckAccess(SendApp, permittedApplications) == OS_CHECK_FAILED) )                        /* COV_OS_XSIGDOUBLECHECK *//* SBSW_OS_FC_PRECONDITION */
...
)
{
  ...
  else
  {
    ...
    if(OS_UNLIKELY(Os_CoreCheckIsLocalCore(schTCore, localCore) == OS_CHECK_FAILED))                                    /* COV_OS_XSIGDOUBLECHECK *//* SBSW_OS_XSIG_CORECHECKISLOCALCORE_004 */
    {
      status = OS_STATUS_CORE_CROSS_DATA;
    }
    else if( OS_UNLIKELY(Os_AppCheckAccess(SendApp, permittedApplications) == OS_CHECK_FAILED) )                        /* COV_OS_XSIGDOUBLECHECK *//* SBSW_OS_FC_PRECONDITION */
...
)
{
  ...
  else
  {
    ...
    if(OS_UNLIKELY(Os_CoreCheckIsLocalCore(schTFromCore, localCore) == OS_CHECK_FAILED))                                /* COV_OS_XSIGDOUBLECHECK *//* SBSW_OS_XSIG_CORECHECKISLOCALCORE_004 */
    {
      status = OS_STATUS_CORE_CROSS_DATA;
    }
    else if(OS_UNLIKELY(Os_CoreCheckIsLocalCore(schTToCore, localCore) == OS_CHECK_FAILED))                             /* COV_OS_XSIGDOUBLECHECK *//* SBSW_OS_XSIG_CORECHECKISLOCALCORE_004 */
    {
      status = OS_STATUS_CORE_CROSS_DATA;
    }
    else if( OS_UNLIKELY(Os_AppCheckAccess(SendApp, permittedApplications_From) == OS_CHECK_FAILED) )                   /* COV_OS_XSIGDOUBLECHECK *//* SBSW_OS_FC_PRECONDITION */
    {
      status = OS_STATUS_CORE_CROSS_DATA;
    }
    else if( OS_UNLIKELY(Os_AppCheckAccess(SendApp, permittedApplications_To) == OS_CHECK_FAILED) )                     /* COV_OS_XSIGDOUBLECHECK *//* SBSW_OS_FC_PRECONDITION */
    {
      status = OS_STATUS_CORE_CROSS_DATA;
    }
    else
...
)
{
  ...
  OS_IGNORE_UNREF_PARAM(SendApp);                                                                                       /* PRQA S 3112 */ /* MD_Os_3112 */
...
)
{
  ...
  else
  {
    ...
    if(OS_UNLIKELY(Os_CoreCheckIsLocalCore(isrCore, localCore) == OS_CHECK_FAILED))                                     /* COV_OS_XSIGDOUBLECHECK *//* SBSW_OS_XSIG_CORECHECKISLOCALCORE_005 */
    {
      status = OS_STATUS_CORE_CROSS_DATA;
    }
    else
...
)
{
  ...
  else
  {
    ...
    if(OS_UNLIKELY(Os_CoreCheckIsAsrCore(targetCore) == OS_CHECK_FAILED))                                               /* COV_OS_XSIGDOUBLECHECK *//* SBSW_OS_XSIG_CORECHECKISASRCORE_001 */
    {
      status = OS_STATUS_CORE_CROSS_DATA;
    }
    else
    {
      ...
      targetAsrCore = Os_Core2AsrCore(targetCore);                                                                      /* SBSW_OS_XSIG_CORE2ASRCORE_001 */
      localCore = Os_ThreadGetCore(Os_CoreGetThread());                                                                 /* SBSW_OS_XSIG_THREADGETCORE_001 */

      if(OS_UNLIKELY(Os_CoreCheckIsLocalCore(targetAsrCore, localCore) == OS_CHECK_FAILED))                             /* COV_OS_XSIGDOUBLECHECK *//* SBSW_OS_XSIG_CORECHECKISLOCALCORE_006 */
      {
        status = OS_STATUS_CORE_CROSS_DATA;
      }
      else
...
)
{
  ...
  else
  {
    ...
    if(OS_UNLIKELY(Os_CoreCheckIsLocalCore(coreRef, localCore) == OS_CHECK_FAILED))                                     /* COV_OS_XSIGDOUBLECHECK *//* SBSW_OS_XSIG_CORECHECKISLOCALCORE_006 */
    {
      status = OS_STATUS_CORE_CROSS_DATA;
    }
    else
    {
      /* #20 Call the local API function. */
      OutParameter->ParameterData.Paramuint32 = Os_StackGetKernelStackUsageLocal(coreRef);                              /* SBSW_OS_PWA_PRECONDITION */ /* SBSW_OS_XSIG_STACKGETUSAGE_001 */
      status = OS_STATUS_OK;
    }
  }
...
)
{
  ...
  else
  {
    ...
    if(OS_UNLIKELY(Os_CoreCheckIsLocalCore(appCore, localCore) == OS_CHECK_FAILED))                                     /* COV_OS_XSIGDOUBLECHECK *//* SBSW_OS_XSIG_CORECHECKISLOCALCORE_007 */
    {
      status = OS_STATUS_CORE_CROSS_DATA;
    }
    else
...
)
{
  ...
  else
  {
    ...
    if(OS_UNLIKELY(Os_CoreCheckIsLocalCore(callbackCore, localCore) == OS_CHECK_FAILED))                                /* COV_OS_XSIGDOUBLECHECK *//* SBSW_OS_XSIG_CORECHECKISLOCALCORE_005 */
    {
      status = OS_STATUS_CORE_CROSS_DATA;
    }
...
\ID SBSW_OS_XSIG_HOOKGETTHREAD_001
 ...
 \COUNTERMEASURE \N [CM_OS_HOOKID2HOOK_N]

\ID SBSW_OS_XSIG_CORECHECKISLOCALCORE_001
 \DESCRIPTION    Os_CoreCheckIsLocalCore is called with the return values of Os_TaskGetCore and Os_ThreadGetCore.
 \COUNTERMEASURE \M [CM_OS_TASKGETCORE_M]
                 \M [CM_OS_THREADGETCORE_M]

\ID SBSW_OS_XSIG_CORECHECKISLOCALCORE_002
 \DESCRIPTION    Os_CoreCheckIsLocalCore is called with the return values of Os_CounterGetCore and Os_ThreadGetCore.
 \COUNTERMEASURE \M [CM_OS_TASKGETCORE_M]
                 \M [CM_OS_THREADGETCORE_M]

\ID SBSW_OS_XSIG_CORECHECKISLOCALCORE_003
 \DESCRIPTION    Os_CoreCheckIsLocalCore is called with the return values of Os_AlarmGetCore and Os_ThreadGetCore.
 \COUNTERMEASURE \M [CM_OS_ALARMGETCORE_M]
                 \M [CM_OS_THREADGETCORE_M]

\ID SBSW_OS_XSIG_CORECHECKISLOCALCORE_004
 \DESCRIPTION    Os_CoreCheckIsLocalCore is called with the return values of Os_SchTGetCore and Os_ThreadGetCore.
...
\ID SBSW_OS_XSIG_CORECHECKISLOCALCORE_005
 \DESCRIPTION    Os_CoreCheckIsLocalCore is called with the return values of two Os_ThreadGetCore calls.
 \COUNTERMEASURE \M [CM_OS_THREADGETCORE_M]

\ID SBSW_OS_XSIG_CORECHECKISLOCALCORE_006
 \DESCRIPTION    Os_CoreCheckIsLocalCore is called with the return values of Os_Core2AsrCore and Os_ThreadGetCore.
 \COUNTERMEASURE \M [CM_OS_COREID2CORE_M]
                 \M [CM_OS_THREADGETCORE_M]

\ID SBSW_OS_XSIG_CORECHECKISLOCALCORE_007
 \DESCRIPTION    Os_CoreCheckIsLocalCore is called with the return values of Os_AppGetCore and Os_ThreadGetCore.
...
\ID SBSW_OS_XSIG_APPCHECKMAYTERMINATE_001
 ...
 \COUNTERMEASURE \M [CM_OS_APPID2APP_M]. No check required for Os_ThreadGetCurrentApplication (see CM_OS_THREADGETCURRENTAPPLICATION_N)
...
\ID COV_OS_XSIGCHANNELDIFFICULTTOREACH
   \ACCEPT XX
   \REASON This part of the function is extremely difficult to reach in a runtime test as two cores have inter-
           operate in a very specific way. This case is better handled in unit tests. Risk is expected to be low.
