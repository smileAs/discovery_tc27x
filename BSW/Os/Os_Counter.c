 ...
 *  \param[in]      CallerCore  The core, which is performing this call. Parameter must not be NULL.
...
))
{
  ...
  Os_Assert((Os_StdReturnType)(result <= Counter->Characteristics.MaxAllowedValue));                                    /* PRQA S 3347 */ /* MD_Os_3347 */
...
{
  return Os_ErrExtendedCheck((Os_StdReturnType)(Counter->DriverType == OS_TIMERTYPE_SOFTWARE));
}
...
))
{
  ...
  if(CallerCore == Counter->Core)                                                                                       /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */
  {
...
))
{
  ...
  return counterValue;                                                                                                  /* PRQA S 3347 */ /* MD_Os_3347 */
}                                                                                                                       /* PRQA S 6050 */ /* MD_Os_STCAL */
...
{
  Os_TickType result;
  Os_Assert((Os_StdReturnType)(Value <= Counter->Characteristics.MaxAllowedValue));
  Os_Assert((Os_StdReturnType)(Sub <= Counter->Characteristics.MaxAllowedValue));
  ...
  {
    result += (Counter->Characteristics.MaxAllowedValue + 1);
  }
...
)
{
  ...
  while(OS_LIKELY(job != NULL_PTR))
  {
    ...
    if(OS_LIKELY(Os_CounterIsFutureValue(Counter, job->Dyn->ExpirationTimestamp, now) == 0))                            /* SBSW_OS_FC_PRECONDITION */
    {
      /* #40 Dequeue the job. */
 ...
 * */
      /* #50 Work the job off. */
      job->Callback(job);                                                                                               /* SBSW_OS_CNT_JOB_CALLBACK_001 */
    }
    else
    {
      break;
...
{
  return (P2CONST(Os_TimerSwConfigType, AUTOMATIC, OS_CONST))Counter;                                                   /* PRQA S 0310 */ /* MD_Os_0310 */
}
...
{
  return (P2CONST(Os_TimerPitConfigType, AUTOMATIC, OS_CONST))Counter;                                                  /* PRQA S 0310 */ /* MD_Os_0310 */
}
...
{
  return (P2CONST(Os_TimerHrtConfigType, AUTOMATIC, OS_CONST))Counter;                                                  /* PRQA S 0310 */ /* MD_Os_0310 */
}
...
)
{
  ...
  return absStart;                                                                                                      /* PRQA S 3347 */ /* MD_Os_3347 */
}
...
)
{
  ...
  Os_Assert((Os_StdReturnType)(Counter->DriverType == OS_TIMERTYPE_SOFTWARE));
  ...
  else
  {
    ...
    Os_TimerSwIncrement(Os_Counter2TimerSw(Counter));                                                                   /* SBSW_OS_CNT_TIMERSWFUNCTION_002 */ /* SBSW_OS_FC_PRECONDITION */
...
)
{
  ...
  if(OS_UNLIKELY(Os_ThreadCheckCallContext(currentThread, OS_APICONTEXT_INCREMENTCOUNTER) == OS_CHECK_FAILED))          /* SBSW_OS_CNT_THREADCHECKCALLCONTEXT_001 */
  {
  ...
  else
  {
    ...
    else if(OS_UNLIKELY(Os_CoreCheckIsLocalCore(me->Core, localCore) == OS_CHECK_FAILED))                               /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */ /* SBSW_OS_CNT_CORECHECKISLOCALCORE_001 */
    {
    ...
    else
    {
      ...
      if(OS_LIKELY(status == OS_STATUS_OK))
      {
        ...
        if(Os_ThreadIsTask(currentThread) != 0)                                                                         /* SBSW_OS_CNT_THREADISTASK_001 */
        {
          ...
          if(Os_SchedulerTaskSwitchIsNeeded(scheduler) != 0)                                                            /* SBSW_OS_CNT_SCHEDULERTASKSWITCHISNEEDED_001 */
          {
...
)
{
  ...
  else
  {
    ...
    {
      status = Os_CounterGetUserValueInternal(me, CounterID, localCore, Value);                                         /* SBSW_OS_CNT_COUNTERGETUSERVALUEINTERNAL_001 */
    }
  }
...
)
{
  ...
  else
  {
    ...
    else
    {
      ...
      status = Os_CounterGetUserValueInternal(me, CounterID, localCore, &currentCounterValue);                          /* SBSW_OS_CNT_COUNTERGETUSERVALUEINTERNAL_002 */
...
\ID SBSW_OS_CNT_JOB_CALLBACK_001
...
\ID SBSW_OS_CNT_CORECHECKISLOCALCORE_001
 \DESCRIPTION     Os_CoreCheckIsLocalCore is called with the core derived from a counter which is the return value of
                  Os_CounterId2Counter and the return value of Os_ThreadGetCore.
 \COUNTERMEASURE  \M [CM_OS_COUNTERID2COUNTER_M]
...
\ID SBSW_OS_CNT_COUNTERGETUSERVALUEINTERNAL_001
 \DESCRIPTION     Os_CounterGetUserValueInternal is called with the return values of Os_CounterId2Counter,
                  Os_ThreadGetCore and a pointer passed as argument to the caller. Correctness of the argument is
...
\ID SBSW_OS_CNT_COUNTERGETUSERVALUEINTERNAL_002
 \DESCRIPTION     Os_CounterGetUserValueInternal is called with the return values of Os_CounterId2Counter,
                  Os_ThreadGetCore and a pointer to a local variable.
 \COUNTERMEASURE  \M [CM_OS_COUNTERID2COUNTER_M]
                  \M [CM_OS_THREADGETCORE_M]
