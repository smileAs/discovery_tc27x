 ...
 * ------------------------
 * ISRs which cannot interrupt each other can share memory for their dynamic part (ISRs which cannot interrupt each other
 * are ISRs on the same priority and not-nestable ISRs). Similar to stack sharing (see \ref StackSharing) this can save
 * a lot of memory (e.g. 200 ISRs on the same priority consume only one dynamic ISR part). As a drawback, it is not
...
#if !defined(OS_ISRINT_H)                                                                                               /* PRQA S 0883 */ /* MD_Os_0883 */
# define OS_ISRINT_H
...
#define OS_ISR_CASTDYN_ISR_2_THREAD(isr)    (&((isr).Thread))                                                           /* PRQA S 3453 */ /* MD_MSR_19.7 */



/*! Type cast from Os_IsrConfigType to Os_IsrConfigType by use of base element addressing. */
#define OS_ISR_CASTCONFIG_ISR_2_ISR(isr)               (&(isr))                                                         /* PRQA S 3453 */ /* MD_MSR_19.7 */
 ...
 *  \trace          CREQ-114872
 **********************************************************************************************************************/
FUNC(Os_StatusType, OS_CODE) Os_Api_IsInterruptSourceEnabled(ISRType ISRID, P2VAR(boolean, AUTOMATIC, OS_VAR_NOINIT) IsEnabled);
 ...
 *  \trace          CREQ-114873
 **********************************************************************************************************************/
FUNC(Os_StatusType, OS_CODE) Os_Api_IsInterruptPending(ISRType ISRID, P2VAR(boolean, AUTOMATIC, OS_VAR_NOINIT) IsPending);
