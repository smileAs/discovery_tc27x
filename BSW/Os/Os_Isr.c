...
{
  return (P2VAR(Os_IsrType, AUTOMATIC, OS_VAR_NOINIT)) (Os_IsrGetThread(Isr)->Dyn);                                     /* PRQA S 0310 */ /* MD_Os_0310 */ /* SBSW_OS_FC_PRECONDITION */
}
...
{
  return Os_ErrExtendedCheck((Os_StdReturnType)((Os_Hal_IntIsClearPendingSupported() != 0    ) ||
...
)
{
  ...
  /* #10 Suspend global interrupt state. */
  Os_IntSuspend(&interruptState);                                                                                       /* SBSW_OS_FC_POINTER2LOCAL */

  /* #20 If inter-arrival time is okay: */
  if(OS_LIKELY(Os_TpCheckInterArrival(timingProtection, Os_ThreadGetTpConfig(&Isr->Thread)) != OS_CHECK_FAILED))        /* SBSW_OS_ISR_TPCHECKINTERARRIVAL_001 */ /* SBSW_OS_FC_PRECONDITION */
  {
  ...
  /* #70 Resume global interrupt state. */
...
OS_FUNC_ATTRIBUTE_DEFINITION(void, OS_CODE, OS_NORETURN, Os_IsrEpilogue, (void))
{
  ...
  else
  {
    ...
    if(OS_UNLIKELY(Os_ThreadCheckSpinlocksReleased(isrThread) == OS_CHECK_FAILED))                                      /* COV_OS_HALPLATFORMTESTEDASMULTICORE */ /* SBSW_OS_ISR_THREADCHECKSPINLOCKSRELEASED_001 */
    {
  ...
  if(Os_ThreadIsTask(interruptedThread) != 0)                                                                           /* SBSW_OS_ISR_THREADISTASK_001 */
  {
 ...
 *
 */
OS_FUNC_ATTRIBUTE_DEFINITION(void, OS_CODE, OS_NORETURN, Os_IsrEpilogueTp, (void))
{
  P2CONST(Os_ThreadConfigType, AUTOMATIC, OS_CONST) isrThread;
  P2CONST(Os_ThreadConfigType, AUTOMATIC, OS_CONST) interruptedThread;
  P2CONST(Os_IsrConfigType, AUTOMATIC, OS_CONST) isr;
  ...
  if(OS_UNLIKELY(Os_IsrGetDyn(isr)->IsKilled == TRUE))                                                                  /* COV_OS_INVSTATE */ /* SBSW_OS_ISR_ISRGETDYN_001 */
  {
...
FUNC(Os_StatusType, OS_CODE) Os_Api_DisableInterruptSource(ISRType ISRID)
{
  ...
  else if(OS_UNLIKELY(Os_ThreadCheckCallContext(currentThread, OS_APICONTEXT_ISRDISABLESOURCE) == OS_CHECK_FAILED))     /* COV_OS_NOFUNCCALL *//* SBSW_OS_ISR_THREADCHECKCALLCONTEXT_001 */
  {
  ...
  else
  {
    ...
    if(OS_UNLIKELY(Os_ThreadCheckIsCurrentApplication(                                                                  /* COV_OS_NOFUNCCALL *//* SBSW_OS_ISR_THREADCHECKISCURRENTAPPLICATION_001 */ /* SBSW_OS_ISR_ISRGETTHREAD_001 */ /* SBSW_OS_ISR_THREADGETOWNERAPPLICATION_001 */
                      currentThread,
...
FUNC(Os_StatusType, OS_CODE) Os_Api_EnableInterruptSource(ISRType ISRID, boolean ClearPending)
{
  ...
  /* #10 Perform checks. */
  if(OS_UNLIKELY(Os_IsrCheckClearPendingSupported(ClearPending) == OS_CHECK_FAILED))                                    /* COV_OS_NOFUNCCALL */
  {
  ...
  else if(OS_UNLIKELY(Os_ThreadCheckCallContext(currentThread, OS_APICONTEXT_ISRENABLESOURCE) == OS_CHECK_FAILED))      /* COV_OS_NOFUNCCALL */ /* SBSW_OS_ISR_THREADCHECKCALLCONTEXT_001 */
  {
    status = OS_STATUS_CALLEVEL;
  }
  else if(OS_UNLIKELY(Os_ErrCheckIsBoolean(ClearPending) == OS_CHECK_FAILED))                                           /* COV_OS_NOFUNCCALL */
  {
...
FUNC(Os_StatusType, OS_CODE) Os_Api_ClearPendingInterrupt(ISRType ISRID)
{
  ...
  else if(OS_UNLIKELY(Os_ThreadCheckCallContext(currentThread, OS_APICONTEXT_ISRCLEARPENDING) == OS_CHECK_FAILED))      /* COV_OS_NOFUNCCALL */ /* SBSW_OS_ISR_THREADCHECKCALLCONTEXT_001 */
  {
    status = OS_STATUS_CALLEVEL;
  }
  else
...
)
{
  ...
  else if(OS_UNLIKELY(Os_ThreadCheckCallContext(                                                                        /* COV_OS_NOFUNCCALL */ /* SBSW_OS_ISR_THREADCHECKCALLCONTEXT_001 */
                         currentThread,
  ...
  else
  {
    ...
    else
    {
      ...
      if (Os_Hal_IntIsInterruptSourceEnabled(isr->HwConfig) == 0 )                                                      /* SBSW_OS_ISR_HAL_INTISINTERRUPTSOURCEENABLED_001 */
      {
...
)
{
  ...
  else if(OS_UNLIKELY(Os_ThreadCheckCallContext(currentThread, OS_APICONTEXT_ISINTERRUPTPENDING) == OS_CHECK_FAILED))   /* COV_OS_NOFUNCCALL */ /* SBSW_OS_ISR_THREADCHECKCALLCONTEXT_001 */
  {
    status = OS_STATUS_CALLEVEL;
  }
  else
  {
    ...
    else
    {
      ...
      if (Os_Hal_IntIsInterruptPending(isr->HwConfig) == 0)                                                             /* SBSW_OS_ISR_HAL_INTISINTERRUPTPENDING_001 */
      {
...
\ID SBSW_OS_ISR_HAL_INTENDOFINTERRUPT_001
 \DESCRIPTION    Os_Hal_IntEndOfInterrupt is called with the HwConfig derived from the caller argument Isr. Precondition
                 ensures that the Isr pointer is valid. The compliance of the precondition is check during review.
