 ...
 *  Os_TrapIsNeeded()
 **********************************************************************************************************************/
/*! \brief          Returns whether a trap has to be performed to access OS data.
 *  \details        Performing a trap is typically very time consuming. For this reason the OS tries to prevent it.
 *                  If the current thread is already privileged, the OS does not have to trap. On some platform this
 *                  optimization is not possible, because the hardware does not allow to check whether the current
 *                  thread is privileged efficiently.
 *
 *  \retval         !0  Trap is needed.
 *  \retval         0   Trap is not needed.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE
 ...
 *  \pre            -
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE Os_StdReturnType, OS_CODE, OS_ALWAYS_INLINE, Os_TrapIsNeeded, (void));

/***********************************************************************************************************************
 *  Os_TrapIsCallerPrivileged()
 **********************************************************************************************************************/
/*! \brief          Returns whether the caller is privileged (!0) or not (0).
 *  \details        --no details--
 *
 *  \retval         !0  The caller is privileged.
 *  \retval         0   The caller is not privileged.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE
 ...
 *  \pre            -
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE Os_StdReturnType, OS_CODE, OS_ALWAYS_INLINE, Os_TrapIsCallerPrivileged,
(void));
 ...
 *  \details        This function informs the trace module on entry and exit of the API Os_GetProtectionHookStackUsage().
...
OS_LOCAL_INLINE Os_StdReturnType, OS_CODE, OS_ALWAYS_INLINE, Os_TrapIsNeeded,
(
  void
))
{
  ...
  if(Os_MpIsEnabled() != 0)
  {
    ...
    if(Os_Hal_CoreIsThreadPointerUserReadable() != 0)                                                                   /* COV_OS_HALPLATFORMTHREADPOINTERUSERREADABILITY */
    {
      /* #30 Return 0 if caller is privileged and !0 if not. */
      result = !Os_TrapIsCallerPrivileged();
    }
 ...
 *
 */
OS_FUNC_ATTRIBUTE_DEFINITION(                                                                                           /* COV_OS_HALPLATFORMTRAPALWAYSNEEDED */
OS_LOCAL_INLINE Os_StdReturnType, OS_CODE, OS_ALWAYS_INLINE, Os_TrapIsCallerPrivileged,
(
  void
))                                                                                                                      /* COV_OS_HALPLATFORMTRAPALWAYSNEEDED */
{
 ...
 *
 */
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE Os_StdReturnType, OS_CODE, OS_ALWAYS_INLINE, Os_TrapIsTrapRequired,
(
  boolean IsTrapRequired
))
{
  ...
  /* #10 If memory protection is enabled: */
  if(Os_MpIsEnabled() != 0)                                                                                             /* COV_OS_HALPLATFORMTESTEDASMULTICORE */
  {
    /* #20 If the thread pointer is readable in user mode: */
    if(Os_Hal_CoreIsThreadPointerUserReadable() != 0)                                                                   /* COV_OS_HALPLATFORMTHREADPOINTERUSERREADABILITY */
    {
      /* #30 Return IsTrapRequired. */
...
))
{
  ...
  if(Os_MpIsEnabledForTrusted() != 0)                                                                                   /* COV_OS_UNSUPPORTED88830 */
  {
    ...
    /* #20 Suspend interrupts (up to timing protection). */
    ...
    /* #40 Resume interrupts (up to timing protection). */
  ...
  /* #50 Return caller access rights. */
...
{
  /* #10 Inform the trace module on service function entry. */
  Os_TraceOrtiApiEntry(OsOrtiApiIdInitMemory);

  /* #20 Call the API. */
  Os_Api_InitMemory();

  /* #30 Inform the trace module on service function exit. */
  Os_TraceOrtiApiExit(OsOrtiApiIdInitMemory);
}
...
))
{
  ...
  /* #10 Inform the trace module on service function entry. */
  Os_TraceOrtiApiEntry(OsOrtiApiIdInit);

  /* #20 Call the API. */
  Os_Api_Init();

  /* #30 Inform the trace module on service function exit. */
  Os_TraceOrtiApiExit(OsOrtiApiIdInit);
}
...
)
{
  ...
  /* #10 If a trap is needed: */
  if(Os_TrapIsNeeded() != 0)                                                                                            /* COV_OS_HALPLATFORMTESTEDASMULTICORE */
  {
    Os_TrapPacketType packet;
...
)
{
  ...
  if(Os_TrapIsNeeded() != 0)
  {
    ...
    packet.Parameters[OS_TRAPPARAMINDEX_2].ParamApiType.ParamTryToGetSpinlockRefType =
                                                                (TryToGetSpinlockType*)&userspaceSuccess;               /* PRQA S 0312 */ /* MD_Os_0312_Volatile_Api */ /* SBSW_OS_PWA_LOCAL */
 ...
 * */
  ...
  if(Os_TrapIsTrapRequired(IsTrapRequired) != 0)                                                                        /* COV_OS_TRAPMISSINGTEST */
  {
    Os_TrapPacketType packet;
...
)
{
  ...
  /* #10 If trap is required: */
  if(Os_TrapIsTrapRequired(IsTrapRequired) != 0)                                                                        /* COV_OS_HALPLATFORMTESTEDASMULTICORE */
  {
    Os_TrapPacketType packet;
...
{
  /* #10 If a trap is needed: */
  if(Os_TrapIsNeeded() != 0)                                                                                            /* COV_OS_TRAPALWAYSNEEDED */
  {
    Os_TrapPacketType packet;
...
)
{
  ...
  if(OS_UNLIKELY(Os_ErrCheckPointerIsNotNull(Context) == OS_CHECK_FAILED))                                              /* SBSW_OS_TRAP_ERRCHECKPOINTERISNOTNULL_001 */
  {
  ...
  else
  {
    ...
    if(Os_TrapIsNeeded() != 0)
    {
      ...
      packet.Parameters[OS_TRAPPARAMINDEX_1].ParamApiType.ParamExceptionContextRefType =
                                                               (Os_ExceptionContextType*)&userspaceContext;             /* PRQA S 0312 */ /* MD_Os_0312_Volatile_Api */ /* SBSW_OS_PWA_LOCAL */
...
)
{
  ...
  if(OS_UNLIKELY(Os_ErrCheckPointerIsNotNull(Status) != OS_CHECK_FAILED))                                               /* SBSW_OS_TRAP_ERRCHECKPOINTERISNOTNULL_001 */
  {
...
#  pragma warning 549        /* This suppresses the warning with regard to "condition always true". This can be
                                suppressed here, since this warning is depended on user configurations. Code coverage
                                analysis ensures the execution of both branches. */
# endif
 ...
 *
 */
FUNC(uint32, OS_CODE) GetNumberOfActivatedCores                                                                         /* COV_OS_HALPLATFORMTESTEDASMULTICORE */
(
  void
)
{
  uint32 result;
  Os_StdReturnType tracingEnabled;

  tracingEnabled = Os_TraceIsOrtiServiceTracingEnabled();

  /* #10 If a trap is needed (only in case of tracing enabled): */
  if((Os_TrapIsNeeded() != 0) && (tracingEnabled != 0))                                                                 /* COV_OS_TRAPMISSINGTEST */
  {
    Os_TrapPacketType packet;
...
)
{
  ...
  /* #10 Check that the given pointer is valid. */
  if(OS_UNLIKELY(Os_ErrCheckPointerIsNotNull(InterruptSource) == OS_CHECK_FAILED))                                      /* COV_OS_HALPLATFORMTESTEDASMULTICORE */ /* SBSW_OS_TRAP_ERRCHECKPOINTERISNOTNULL_001 */
  {
    status = OS_STATUS_PARAM_POINTER_1;
  }
  else
  {
    ...
    if(Os_TrapIsNeeded() != 0)                                                                                          /* COV_OS_HALPLATFORMTESTEDASMULTICORE */
    {
      ...
      packet.Parameters[OS_TRAPPARAMINDEX_1].ParamApiType.ParamInterruptSourceIdRefType =
                                                               (Os_InterruptSourceIdType*)&userspaceInterruptSource;    /* PRQA S 0312 */ /* MD_Os_0312_Volatile_Api */ /* SBSW_OS_PWA_LOCAL */
...
)
{
  ...
  /* #10 Check that the given pointer is valid. */
  if(OS_UNLIKELY(Os_ErrCheckPointerIsNotNull(ExceptionSource) == OS_CHECK_FAILED))                                      /* COV_OS_HALPLATFORMTESTEDASMULTICORE */ /* SBSW_OS_TRAP_ERRCHECKPOINTERISNOTNULL_001 */
  {
    status = OS_STATUS_PARAM_POINTER_1;
  }
  else
  {
    ...
    if(Os_TrapIsNeeded() != 0)                                                                                          /* COV_OS_HALPLATFORMTESTEDASMULTICORE */
    {
      ...
      packet.Parameters[OS_TRAPPARAMINDEX_1].ParamApiType.ParamExceptionSourceIdRefType =
                                                               (Os_ExceptionSourceIdType*)&userspaceExceptionSource;    /* PRQA S 0312 */ /* MD_Os_0312_Volatile_Api */ /* SBSW_OS_PWA_LOCAL */
...
)
{
  ...
  /* #10 Check that the given pointer is valid. */
  if(OS_UNLIKELY(Os_ErrCheckPointerIsNotNull(Success) == OS_CHECK_FAILED))                                              /* COV_OS_NOFUNCCALL *//* SBSW_OS_TRAP_ERRCHECKPOINTERISNOTNULL_001 */
  {
...
)
{
  ...
  /* #10 Check that the given pointer is valid. */
  if(OS_UNLIKELY(Os_ErrCheckPointerIsNotNull(IsEnabled) == OS_CHECK_FAILED))                                            /* COV_OS_NOFUNCCALL *//* SBSW_OS_TRAP_ERRCHECKPOINTERISNOTNULL_001 */
  {
    status = OS_STATUS_PARAM_POINTER_2;
  }
  else
  {
    ...
    if(Os_TrapIsNeeded() != 0)
    {
      ...
      packet.Parameters[OS_TRAPPARAMINDEX_1].ParamApiType.ParamISRType = ISRID;                                         /* SBSW_OS_PWA_LOCAL */
      packet.Parameters[OS_TRAPPARAMINDEX_2].ParamApiType.ParambooleanRefType = (boolean*)&userspaceIsEnabled;          /* PRQA S 0312 */ /* MD_Os_0312_Volatile_Api */ /* SBSW_OS_PWA_LOCAL */
...
)
{
  ...
  /* #10 Check that the given pointer is valid. */
  if(OS_UNLIKELY(Os_ErrCheckPointerIsNotNull(IsPending) == OS_CHECK_FAILED))                                            /* COV_OS_NOFUNCCALL *//* SBSW_OS_TRAP_ERRCHECKPOINTERISNOTNULL_001 */
  {
    status = OS_STATUS_PARAM_POINTER_2;
  }
  else
  {
    ...
    if(Os_TrapIsNeeded() != 0)
    {
      ...
      packet.Parameters[OS_TRAPPARAMINDEX_1].ParamApiType.ParamISRType = ISRID;                                         /* SBSW_OS_PWA_LOCAL */
      packet.Parameters[OS_TRAPPARAMINDEX_2].ParamApiType.ParambooleanRefType = (boolean*)&userspaceIsPending;          /* PRQA S 0312 */ /* MD_Os_0312_Volatile_Api */ /* SBSW_OS_PWA_LOCAL */
...
FUNC(StatusType, OS_CODE) GetTaskID(TaskRefType TaskID)
{
  ...
  /* #10 Check that the given pointer is valid. */
  if(OS_UNLIKELY(Os_ErrCheckPointerIsNotNull(TaskID) == OS_CHECK_FAILED))                                               /* SBSW_OS_TRAP_ERRCHECKPOINTERISNOTNULL_001 */
  {
    status = OS_STATUS_PARAM_POINTER_1;
  }
  else
  {
    ...
    if(Os_TrapIsNeeded() != 0)
    {
      ...
      packet.Parameters[OS_TRAPPARAMINDEX_1].ParamApiType.ParamTaskRefType = (TaskType*)&userspaceTaskId;               /* PRQA S 0312 */ /* MD_Os_0312_Volatile_Api */ /* SBSW_OS_PWA_LOCAL */
...
FUNC(StatusType, OS_CODE) GetTaskState(TaskType TaskID, TaskStateRefType State)
{
  ...
  /* #10 Check that the given pointer is valid. */
  if(OS_UNLIKELY(Os_ErrCheckPointerIsNotNull(State) == OS_CHECK_FAILED))                                                /* SBSW_OS_TRAP_ERRCHECKPOINTERISNOTNULL_001 */
  {
    status = OS_STATUS_PARAM_POINTER_2;
  }
  else
  {
    ...
    if(Os_TrapIsNeeded() != 0)
    {
      ...
      packet.Parameters[OS_TRAPPARAMINDEX_1].ParamApiType.ParamTaskType = TaskID;                                       /* SBSW_OS_PWA_LOCAL */
      packet.Parameters[OS_TRAPPARAMINDEX_2].ParamApiType.ParamTaskStateRefType = (TaskStateType*)&userspaceState;      /* PRQA S 0312 */ /* MD_Os_0312_Volatile_Api */ /* SBSW_OS_PWA_LOCAL */
...
)
{
  ...
  /* #10 Check that the given pointer is valid. */
  if(OS_UNLIKELY(Os_ErrCheckPointerIsNotNull(Mask) == OS_CHECK_FAILED))                                                 /* SBSW_OS_TRAP_ERRCHECKPOINTERISNOTNULL_001 */
  {
    status = OS_STATUS_PARAM_POINTER_2;
  }
  else
  {
    ...
    if(Os_TrapIsNeeded() != 0)
    {
      ...
      packet.Parameters[OS_TRAPPARAMINDEX_1].ParamApiType.ParamTaskType = TaskID;                                       /* SBSW_OS_PWA_LOCAL */
      packet.Parameters[OS_TRAPPARAMINDEX_2].ParamApiType.ParamEventMaskRefType = (EventMaskType*)&userspaceMask;       /* PRQA S 0312 */ /* MD_Os_0312_Volatile_Api */ /* SBSW_OS_PWA_LOCAL */
...
)
{
  ...
  /* #10 Check that the given pointer is valid. */
  if(OS_UNLIKELY(Os_ErrCheckPointerIsNotNull(Value) == OS_CHECK_FAILED))                                                /* SBSW_OS_TRAP_ERRCHECKPOINTERISNOTNULL_001 */
  {
    status = OS_STATUS_PARAM_POINTER_2;
  }
  else
  {
    ...
    if(Os_TrapIsNeeded() != 0)
    {
      ...
      packet.Parameters[OS_TRAPPARAMINDEX_1].ParamApiType.ParamCounterType = CounterID;                                 /* SBSW_OS_PWA_LOCAL */
      packet.Parameters[OS_TRAPPARAMINDEX_2].ParamApiType.ParamTickRefType = (TickType*)&userspaceValue;                /* PRQA S 0312 */ /* MD_Os_0312_Volatile_Api */ /* SBSW_OS_PWA_LOCAL */
...
)
{
  ...
  else if(OS_UNLIKELY(Os_ErrCheckPointerIsNotNull(ElapsedValue) == OS_CHECK_FAILED))                                    /* SBSW_OS_TRAP_ERRCHECKPOINTERISNOTNULL_001 */
  {
  ...
  else
  {
    ...
    if(Os_TrapIsNeeded() != 0)
    {
      ...
      packet.Parameters[OS_TRAPPARAMINDEX_1].ParamApiType.ParamCounterType = CounterID;                                 /* SBSW_OS_PWA_LOCAL */
      packet.Parameters[OS_TRAPPARAMINDEX_2].ParamApiType.ParamTickRefType = (TickType*)&userspaceValue;                /* PRQA S 0312 */ /* MD_Os_0312_Volatile_Api */ /* SBSW_OS_PWA_LOCAL */
      packet.Parameters[OS_TRAPPARAMINDEX_3].ParamApiType.ParamTickRefType = (TickType*)&userspaceElapsedValue;         /* PRQA S 0312 */ /* MD_Os_0312_Volatile_Api */ /* SBSW_OS_PWA_LOCAL */
...
)
{
  ...
  /* #10 Check that the given pointer is valid. */
  if(OS_UNLIKELY(Os_ErrCheckPointerIsNotNull(Info) == OS_CHECK_FAILED))                                                 /* SBSW_OS_TRAP_ERRCHECKPOINTERISNOTNULL_001 */
  {
    status = OS_STATUS_PARAM_POINTER_2;
  }
  else
  {
    ...
    if(Os_TrapIsNeeded() != 0)
    {
      ...
      packet.Parameters[OS_TRAPPARAMINDEX_2].ParamApiType.ParamAlarmBaseRefType = (AlarmBaseType*)&userspaceInfo;       /* PRQA S 0312 */ /* MD_Os_0312_Volatile_Api */ /* SBSW_OS_PWA_LOCAL */
...
)
{
  ...
  /* #10 Check that the given pointer is valid. */
  if(OS_UNLIKELY(Os_ErrCheckPointerIsNotNull(Tick) == OS_CHECK_FAILED))                                                 /* SBSW_OS_TRAP_ERRCHECKPOINTERISNOTNULL_001 */
  {
    status = OS_STATUS_PARAM_POINTER_2;
  }
  else
  {
    ...
    if(Os_TrapIsNeeded() != 0)
    {
      ...
      packet.Parameters[OS_TRAPPARAMINDEX_1].ParamApiType.ParamAlarmType = AlarmID;                                     /* SBSW_OS_PWA_LOCAL */
      packet.Parameters[OS_TRAPPARAMINDEX_2].ParamApiType.ParamTickRefType = (TickType*)&userspaceTick;                 /* PRQA S 0312 */ /* MD_Os_0312_Volatile_Api */ /* SBSW_OS_PWA_LOCAL */
...
)
{
  ...
  /* #10 Check that the given pointer is valid. */
  if(OS_UNLIKELY(Os_ErrCheckPointerIsNotNull(ScheduleStatus) == OS_CHECK_FAILED))                                       /* SBSW_OS_TRAP_ERRCHECKPOINTERISNOTNULL_001 */
  {
    status = OS_STATUS_PARAM_POINTER_2;
  }
  else
  {
    ...
    if(Os_TrapIsNeeded() != 0)
    {
      ...
      packet.Parameters[OS_TRAPPARAMINDEX_2].ParamApiType.ParamScheduleTableStatusRefType =
                                                                  (ScheduleTableStatusType*)&userspaceScheduleStatus;   /* PRQA S 0312 */ /* MD_Os_0312_Volatile_Api */ /* SBSW_OS_PWA_LOCAL */
...
)
{
  ...
  /* #10 If a trap is needed: */
  if(Os_TrapIsNeeded() != 0)                                                                                            /* COV_OS_HALPLATFORMTRAPALWAYSNEEDED */
  {
    Os_TrapPacketType packet;
...
)
{
  ...
  /* #10 Check that the given pointer is valid. */
  if(OS_UNLIKELY(Os_ErrCheckPointerIsNotNull(Value) == OS_CHECK_FAILED))                                                /* COV_OS_NOFUNCCALL *//* SBSW_OS_TRAP_ERRCHECKPOINTERISNOTNULL_001 */
  {
    status = OS_STATUS_PARAM_POINTER_2;
  }
  else
  {
    ...
    if(Os_TrapIsNeeded() != 0)
    {
      ...
      packet.Parameters[OS_TRAPPARAMINDEX_2].ParamApiType.ParamApplicationStateRefType =
                                                                  (ApplicationStateType*)&userspaceValue;               /* PRQA S 0312 */ /* MD_Os_0312_Volatile_Api */ /* SBSW_OS_PWA_LOCAL */
...
)                                                                                                                       /* COV_OS_UNSUPPORTED93813 */
{
  ...
  /* #10 If a trap is needed: */
  if(Os_TrapIsNeeded() != 0)                                                                                            /* COV_OS_UNSUPPORTED93813 */
  {
    Os_TrapPacketType packet;
...
)
{
  ...
  /* #10 Check that the given pointer is valid. */
  if(OS_UNLIKELY(Os_ErrCheckPointerIsNotNull(ErrorRef) == OS_CHECK_FAILED))                                             /* COV_OS_NOFUNCCALL *//* SBSW_OS_TRAP_ERRCHECKPOINTERISNOTNULL_001 */
  {
    status = OS_STATUS_PARAM_POINTER_1;
  }
  else
...
)
{
  ...
  switch(Packet->TrapId)
  {
    ...
    case OS_TRAPID_IOC08RECEIVE:                                                                                        /* COV_OS_HALPLATFORMUNSUPPORTED8082 */
    ...
    case OS_TRAPID_IOC08EMPTYQUEUE:                                                                                     /* COV_OS_HALPLATFORMUNSUPPORTED8082 */
      Packet->ReturnValue.ReturnStd_ReturnType =                                                                        /* SBSW_OS_PWA_PRECONDITION */
    ...
    case OS_TRAPID_IOC16RECEIVE:                                                                                        /* COV_OS_HALPLATFORMUNSUPPORTED8082 */
      Packet->ReturnValue.ReturnStd_ReturnType =                                                                        /* SBSW_OS_PWA_PRECONDITION */
    ...
    case OS_TRAPID_IOC16EMPTYQUEUE:                                                                                     /* COV_OS_HALPLATFORMUNSUPPORTED8082 */
      Packet->ReturnValue.ReturnStd_ReturnType =                                                                        /* SBSW_OS_PWA_PRECONDITION */
    ...
    case OS_TRAPID_IOC08MULTIRECEIVE:                                                                                   /* COV_OS_HALPLATFORMUNSUPPORTED8082 */
      Packet->ReturnValue.ReturnStd_ReturnType =                                                                        /* SBSW_OS_PWA_PRECONDITION */
    ...
    case OS_TRAPID_IOC08EMPTYMULTIQUEUE:                                                                                /* COV_OS_HALPLATFORMUNSUPPORTED8082 */
      Packet->ReturnValue.ReturnStd_ReturnType =                                                                        /* SBSW_OS_PWA_PRECONDITION */
    ...
    case OS_TRAPID_IOC16MULTIRECEIVE:                                                                                   /* COV_OS_HALPLATFORMUNSUPPORTED8082 */
      Packet->ReturnValue.ReturnStd_ReturnType =                                                                        /* SBSW_OS_PWA_PRECONDITION */
    ...
    case OS_TRAPID_IOC16EMPTYMULTIQUEUE:                                                                                /* COV_OS_HALPLATFORMUNSUPPORTED8082 */
      Packet->ReturnValue.ReturnStd_ReturnType =                                                                        /* SBSW_OS_PWA_PRECONDITION */
    ...
    case OS_TRAPID_IOC32MULTIRECEIVE:                                                                                   /* COV_OS_HALPLATFORMUNSUPPORTED8082 */
      Packet->ReturnValue.ReturnStd_ReturnType =                                                                        /* SBSW_OS_PWA_PRECONDITION */
    ...
    case OS_TRAPID_IOC32EMPTYMULTIQUEUE:                                                                                /* COV_OS_HALPLATFORMUNSUPPORTED8082 */
      Packet->ReturnValue.ReturnStd_ReturnType =                                                                        /* SBSW_OS_PWA_PRECONDITION */
    ...
    case OS_TRAPID_IOCREFMULTIRECEIVE:                                                                                  /* COV_OS_HALPLATFORMUNSUPPORTED8082 */
      Packet->ReturnValue.ReturnStd_ReturnType =                                                                        /* SBSW_OS_PWA_PRECONDITION */
    ...
    case OS_TRAPID_IOCREFEMPTYMULTIQUEUE:                                                                               /* COV_OS_HALPLATFORMUNSUPPORTED8082 */
      Packet->ReturnValue.ReturnStd_ReturnType =                                                                        /* SBSW_OS_PWA_PRECONDITION */
    ...
    case OS_TRAPID_CHECKTASKMEMORYACCESS:                                                                               /* COV_OS_TRAPMISSINGTEST */
    ...
    case OS_TRAPID_CHECKISRMEMORYACCESS:                                                                                /* COV_OS_TRAPMISSINGTEST */
      Packet->ReturnValue.ReturnAccessType =                                                                            /* SBSW_OS_PWA_PRECONDITION */
  ...
  /* #40 Restore write access of caller. */
  Os_TrapMpSwitchToCaller(callerAccessRights);                                                                          /* SBSW_OS_TRAPMPSWITCHTOCALLER_001 */
...
\ID COV_OS_TRAPMISSINGTEST
   ...
   \REASON The condition could be evaluated to true and false with additional tests. However, as very similar functions
           ...
           A work item exists to remind the developers about the missing tests: WORKITEM9936: New tests to cover all
           paths in trap functions
