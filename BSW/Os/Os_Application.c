 ...
 *  \pre            -
 **********************************************************************************************************************/
OS_LOCAL FUNC(P2CONST(Os_AppConfigType, AUTOMATIC, OS_CONST), OS_CODE) Os_AppObjectGetApp
(
 ...
 *  \pre            -
 **********************************************************************************************************************/
OS_LOCAL FUNC(Os_AppAccessMaskType, OS_CODE) Os_AppObjectAccessIsAllowed
(
 ...
 **********************************************************************************************************************/
OS_LOCAL FUNC(void, OS_CODE) Os_AppKillMembers
(
  P2CONST(Os_AppConfigType, AUTOMATIC, OS_CONST) App
);
 ...
 *  \pre            Concurrent access to given object is prevented by caller.
 **********************************************************************************************************************/
OS_LOCAL FUNC(void, OS_CODE) Os_AppKillInterruptedMemberThreads
(
  P2CONST(Os_AppConfigType, AUTOMATIC, OS_CONST) App
);
...
{
  return Os_ErrExtendedCheck(Os_AppIsKernelApp(App) == 0);                                                              /* SBSW_OS_FC_PRECONDITION */
}
...
{
  return Os_ErrExtendedCheck(((Os_AppIsTrusted(TerminatorApp)) || (TerminatorApp == TargetApp)));                       /* SBSW_OS_FC_PRECONDITION */
}
...
)
{
  ...
  switch(ObjectType)
  {
    case OBJECT_TASK:
      {
        TaskType objectId = (TaskType)ObjectID;
    ...
    case OBJECT_ISR:
      {
        ISRType objectId = (ISRType)ObjectID;
    ...
    case OBJECT_ALARM:
      {
        AlarmType objectId = (AlarmType)ObjectID;
    ...
    case OBJECT_COUNTER:
      {
        CounterType objectId = (CounterType)ObjectID;
    ...
    case OBJECT_SCHEDULETABLE:
      {
        ScheduleTableType objectId = (ScheduleTableType)ObjectID;
...
)
{
  ...
  switch(ObjectType)
  {
    ...
    case OBJECT_RESOURCE:
      {
        ResourceType objectId = (ResourceType)ObjectID;
    ...
    case OBJECT_SPINLOCK:
      {
        SpinlockIdType objectId = (SpinlockIdType)ObjectID;
 ...
 *
 */
OS_LOCAL_INLINE FUNC(void, OS_CODE) Os_AppKillMembers
(
  P2CONST(Os_AppConfigType, AUTOMATIC, OS_CONST) App
)
{
...
{
  /* #10 Check that the given application is an user application. */
  Os_Assert((Os_StdReturnType)(Os_AppIsKernelApp(App) == 0));                                                           /* SBSW_OS_FC_PRECONDITION */
  ...
  if(RestartOption == RESTART)
  {
    ...
    Os_Assert((Os_StdReturnType)(App->RestartTask != NULL_PTR));
...
)
{
  ...
  else
  {
    ...
    if(OS_LIKELY(Os_AppIsMemberOfCore(targetApp, Os_ThreadGetCore(currentThread)) != 0))                                /* COV_OS_HALPLATFORMTESTEDASMULTICORE */ /* SBSW_OS_APP_APPISMEMBEROFCORE_001 */ /* SBSW_OS_APP_THREADGETCORE_001 */
    {
...
)
{
  ...
  /* #10 Perform error checks. */
  if(OS_UNLIKELY(Os_AppCheckId(ApplID) == OS_CHECK_FAILED ))
  {
    result = NO_ACCESS;
  }
  else
  {
    ...
    if(Os_AppObjectAccessIsAllowed(ApplID, ObjectType, ObjectID) != 0)
    {
      result = ACCESS;
    }
    else
...
)
{
  ...
  /* #10 Perform error checks. */
  if(OS_UNLIKELY(Os_ThreadCheckCallContext(                                                                             /* SBSW_OS_APP_THREADCHECKCALLCONTEXT_001 */
...
)
{
  ...
  else
  {
    ...
    /* #30 If the given application is mapped to the local core: */
    if(OS_LIKELY(Os_AppIsMemberOfCore(targetApp, Os_ThreadGetCore(currentThread)) != 0))                                /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */ /* SBSW_OS_APP_APPISMEMBEROFCORE_001 */ /* SBSW_OS_APP_THREADGETCORE_001 */
    {
...
)
{
  ...
  else
  {
    ...
    else
    {
      ...
      if(Os_ThreadIsTask(currentThread) != 0)                                                                           /* SBSW_OS_APP_THREADISTASK_001 */
      {
        ...
        if((Os_SchedulerTaskSwitchIsNeeded(scheduler) != 0))                                                            /* SBSW_OS_APP_SCHEDULERTASKSWITCHISNEEDED_001 */
        {
...
\ID SBSW_OS_APP_DYN_001
 \DESCRIPTION    Write access to the dynamic data of an application. The application pointer is provided as a parameter, validity is assured by precondition.
                 The compliance to the precondition is checked by review.
 \COUNTERMEASURE \M [CM_OS_APPDYN_M]

\ID SBSW_OS_APP_DYN_002
 \DESCRIPTION    Write access to the dynamic data of an application. The application pointer is provided as a return value of Os_ThreadGetCurrentApplication.
...
\ID SBSW_OS_APP_APPTERMINATEAPPLICATION_002
 \DESCRIPTION    Os_AppTerminateApplication is called with the return values of Os_AppId2App() and a parameter argument
                 of the caller function. The correctness of the argument is ensured by a precondition. The precondition is checked
                 by review.
...
\ID SBSW_OS_APPGETID_002
 \DESCRIPTION    Os_AppGetId is called with the return value of Os_AppObjectGetApp(). That function always returns a valid pointer or
                 NULL_PTR. Check against NULL_PTR exists.
...
\ID SBSW_OS_APP_APPISMEMBEROFCORE_001
 \DESCRIPTION    Os_AppIsMemberOfCore is called with the return values of Os_AppId2App and Os_ThreadGetCore.
 \COUNTERMEASURE \M [CM_OS_APPID2APP_M]
...
\ID SBSW_OS_APP_CORECHECKISLOCALCORE_001
 \DESCRIPTION    Os_CoreCheckIsLocalCore is called with the Core member of an App instance and a value which are
                 passed as arguments to the caller. Precondition ensures that the
                 App pointer and the CallerCore is valid. The compliance of the precondition is check during review.
 \COUNTERMEASURE \M [CM_OS_APPGETCORE_M]

\ID SBSW_OS_APP_APPKILLMEMBERTHREAD_001
 \DESCRIPTION    Os_AppKillMemberThread is called with the return value of Os_CoreInterruptedThreadsGet and the App parameter of the caller,
                 which is assured to be valid by precondition. The precondition is checked by review.
