 ...
 *
 *  \note         SPEC-63718  Deviation: The OS performs a shutdown instead of application termination,
 *                            if there is no task or ISR which can be identified to be responsible for the protection
 *                            violation. Because the OS does not know, whether the application can handle application
 *                            termination.
 **********************************************************************************************************************/
...
)
{
  ...
  else if(OS_UNLIKELY(Os_CoreOsIsStarted(Os_ThreadGetCore(currentThread)) == 0) &&
                      (UserReaction != PRO_IGNORE_EXCEPTION))                                                           /* SBSW_OS_ERROR_COREOSISSTARTED_001 */ /* SBSW_OS_ERROR_THREADGETCORE_001 */
  {
    result = PRO_SHUTDOWN;
  }
  ...
  else if(OS_UNLIKELY((Os_CoreCriticalUserSectionIsActive(Os_ThreadGetCore(currentThread)) == TRUE) &&
                      (UserReaction != PRO_IGNORE_EXCEPTION)))                                                          /* SBSW_OS_ERROR_THREADGETCORE_001 */ /* SBSW_OS_ERROR_CORECRITICALUSERSECTIONISACTIVE_001 */
  {
    result = PRO_SHUTDOWN;
  }  
  ...
  else if(OS_UNLIKELY((Os_CoreIsExceptionContextSupported() == 0) && (UserReaction == PRO_IGNORE_EXCEPTION)))           /* COV_OS_NOFUNCCALL */
  {
    result = PRO_SHUTDOWN;
  }
...
)
{
  ...
  /* #10 Call panic hook if the responsible thread cannot be resolved. */
  responsibleThread = Os_CoreThreadGetByContext(Os_ThreadGetCore(currentThread), OS_CALLCONTEXT_TERMINATEABLE);         /* SBSW_OS_ERROR_CORETHREADGETBYCONTEXT_001 */ /* SBSW_OS_ERROR_THREADGETCORE_001 */
  if(OS_UNLIKELY(responsibleThread == NULL_PTR))                                                                        /* COV_OS_INVSTATE */
  {
    /* As there is always a task, this case indicates an consistency error. */
...
FUNC(Os_ExceptionContextRefType, OS_CODE) Os_ErrorGetParameter_ExceptionContextRefType(Os_ErrorParmIndexType ParamIndex)
{
  ...
  /* #10 Fetch requested error parameter from core and return it. */
  return Os_CoreGetLastError(Os_ThreadGetCore(Os_TrapCoreGetThread()))                                                  /* SBSW_OS_ERROR_COREGETLASTERROR_001 */ /* SBSW_OS_ERROR_THREADGETCORE_002 */
      ->Parameters.Parameter[ParamIndex].ParamExceptionContextRefType;                                                  /* PRQA S 3689 */ /* MD_Os_3689 */
}
...
FUNC(Os_BarrierIdType, OS_CODE) Os_ErrorGetParameter_BarrierIdType(Os_ErrorParmIndexType ParamIndex)                    /* COV_OS_UNSUPPORTED93797 */
{
  ...
  /* #10 Fetch requested error parameter from core and return it. */
  return Os_CoreGetLastError(Os_ThreadGetCore(Os_TrapCoreGetThread()))                                                  /* SBSW_OS_ERROR_COREGETLASTERROR_001 */ /* SBSW_OS_ERROR_THREADGETCORE_002 */
      ->Parameters.Parameter[ParamIndex].ParamBarrierIdType;                                                            /* PRQA S 3689 */ /* MD_Os_3689 */
}
