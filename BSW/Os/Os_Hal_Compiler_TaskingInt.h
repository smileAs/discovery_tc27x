...
#if !defined(OS_HAL_COMPILER_TASKINGINT_H)
 ...
 *  \pre           -
 */
# define Os_Hal_Mtcr(x, y)        __mtcr((sint32)(x),(sint32)(y))
 ...
 *                 Os_UnhandledExc is called with the trap class and the trap identification
...
{ \
  ...
  pcxi = Os_Hal_SysCall(tin, pcxi, a4); \
 ...
 *                 are disabled until a specific level by using the key word __bisr_. The core function
 *                 Os_IsrRun is called to handle the Cat2 ISR with the pointer to the interrupt configuration
 *                 data as parameter.
 *  \param[in]     core          On which core the entry is registered. It must be in range [0, 6].
...
void __interrupt(level) __bisr_(systemlevel) __vector_table(core) osIsrLevel_##level##_Core##core(void) \
{ \
  Os_IsrRun((Os_IsrConfigType*)&isrconfig); \
}
...
# define Os_Hal_DeclareTrapFunction(name, parameters, id)   \
         __syscallfunc(id) void name parameters
