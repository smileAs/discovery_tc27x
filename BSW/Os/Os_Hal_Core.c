...
\ID SBSW_OS_HAL_PWA_PCXI_INTERRUPT
 \DESCRIPTION    A write access is performed on a pointer, which points to the memory address of the CSA frame. The
                 memory address is derived from the PCXI register. This function is called within the trap handler,
                 where the PCXI register always contains a valid value. Thus, this write access is always performed
                 on a valid pointer.
 \COUNTERMEASURE \N No measure is required, because its validity is ensured by code review.

\ID SBSW_OS_HAL_PWA_PCXI_SERVICEFUNCTION
 \DESCRIPTION    A write access is performed on a pointer, which points to the memory address of the CSA frame. The
                 memory address is derived from the InitPCXI or PCXI of the context, which is initialized previously
                 by calling Os_Hal_ContextInit in the core code.
 \COUNTERMEASURE \N No measure is required, because its validity is ensured by code review.
...
\ID SBSW_OS_HAL_FC_MPUCONFIG
 ...
 \COUNTERMEASURE \N No measure required, because this pointer will not be used for write operation by the
                 callee.
