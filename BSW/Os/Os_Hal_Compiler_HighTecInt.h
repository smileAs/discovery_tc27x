...
#if !defined(OS_HAL_COMPILER_HIGHTECINT_H)
 ...
 *                 are disabled until a specific level by using the assembly instruction bisr. The core function
...
# define Os_Hal_Cat2InterruptEntry(core, level, systemlevel, isrconfig) \
  ...
  __asm ("osIsrLevel_" #level "_Core" #core ": bisr " OS_HAL_EXPAND(systemlevel)); \
  __asm ("movh.a  %a4, hi:" #isrconfig); \
  __asm ("lea  %a4, [%a4]lo:" #isrconfig); \
  __asm ("call Os_IsrRun"); \
  __asm ("rslcx"); \
  __asm ("rfe");
