 ...
 **********************************************************************************************************************/
FUNC(void, OS_CODE) Os_TpThreadInitInternal                                                                             /* PRQA S 3447 */ /* MD_Os_3447 */
(
 ...
 *  \pre          The timing protection of the owner core is initialized.
 **********************************************************************************************************************/
FUNC(void, OS_CODE) Os_TpThreadKillInternal                                                                             /* PRQA S 3447 */ /* MD_Os_3447 */
(
 ...
 *
 */
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE void, OS_CODE, OS_ALWAYS_INLINE, Os_TpThreadReInitInternal,
(
  P2CONST(Os_TpOwnerThreadConfigType, AUTOMATIC, OS_CONST) Owner,
  P2VAR(  Os_TpOwnerThreadType,       AUTOMATIC, OS_CONST) Dyn
))
...
)
{
  if(Os_TpIsEnabled() != 0)
  {
    Os_Assert((Os_StdReturnType)(Os_ThreadGetTpConfig(InitialThread)->ThreadKind == OS_TPTHREADKIND_OWNER));            /* SBSW_OS_FC_PRECONDITION */
 ...
 *
  ...
  /* #10 Add passed time to inter-arrival time snapshot. */
  ...
  /* #30 If we have found a budget owner: */
  if(current->Dyn->MonitoredBudgets != NULL_PTR)
  {
    ...
    /* #40 If we have a thread which has exhausted its budget: */
    if(OS_UNLIKELY(Os_CounterIsFutureValue(counter, deadline, now) == 0))                                               /* SBSW_OS_TP_COUNTERISFUTUREVALUE_001 */
    {
      ...
      /* #80 Otherwise, call protection error. */
  ...
  /* #90 Ensure that the next interrupt is in time. */
  {
    P2CONST(Os_CounterConfigType, AUTOMATIC, OS_CONST) counter;
    Os_TickType maxAllowedValue;
    Os_TickType compareValue;
