...
# if defined(NMOSEK_SLEEPACK_NOTIFICATION_ENABLED)
#  define NMOSEK_SID_GETPREPARESENDSLEEPACK      (0xD2u) /*!< Service ID: NmOsek_GetPrepareSendSleepAck() */
#  define NMOSEK_SID_GETSENDSLEEPACK             (0xD3u) /*!< Service ID: NmOsek_GetSendSleepAck() */
#  define NMOSEK_SID_SENDSLEEPACK                (0xD4u) /*!< Service ID: NmOsek_SendSleepAck() */
# endif
...
# if defined(NMOSEK_SLEEPACK_NOTIFICATION_ENABLED)
/**********************************************************************************************************************
 *  NmOsek_GetPrepareSendSleepAck()
 *********************************************************************************************************************/
/*! \brief        This API returns the current status of the Prepare Send Sleep Ack flag
 *  \details      Return the value of NmOsek_PrepSendSleepAck for the given channel.
 *                NmOsek_PrepSendSleepAck is set to 1 if and only if the Sleep Acknowledge Bit may be set to 1 by the
 *                current RING message sent by the node.
 *                The flag is being set to 1 when the first RING message that may contain the Sleep Acknowledge Bit set
 *                to 1 is being prepared for transmission in the context of NmOsek_MainFunction, shortly before the
 *                transmission request (CanIf_Transmit()).
 *                However, the Application may prevent setting the Sleep Acknowledge Bit through the
 *                Appl_NmOsek_SendSleepAck callback function.
 *  \param[in]    nmChannelHandle         Identification of the NM-channel
 *  \return       TRUE  Prepare Send Sleep Ack flag is set
 *  \return       FALSE Prepare Send Sleep Ack flag is not set
 *  \context      TASK|ISR1|ISR2
 *  \reentrant    TRUE
 *  \synchronous  TRUE
 *  \config       defined(NMOSEK_SLEEPACK_NOTIFICATION_ENABLED)
 *  \pre          -
 *  \note         Called by Application
 *********************************************************************************************************************/
extern FUNC(boolean, NMOSEK_CODE) NmOsek_GetPrepareSendSleepAck(CONST(NetworkHandleType, AUTOMATIC) nmChannelHandle);

/**********************************************************************************************************************
 *  NmOsek_GetSendSleepAck()
 *********************************************************************************************************************/
/*! \brief        This API returns the current status of the Send Sleep Ack flag
 *  \details      Return the value of Sleep Ack flag for the given channel.
 *                The flag is set to 1 if and only if the Sleep Acknowledge Bit has been sent or is about to be sent
 *                within the most recent RING message sent by the node.
 *  \param[in]    nmChannelHandle         Identification of the NM-channel
 *  \return       TRUE  Send Sleep Ack flag is set
 *  \return       FALSE Send Sleep Ack flag is not set
 *  \context      TASK|ISR1|ISR2
 *  \reentrant    TRUE
 *  \synchronous  TRUE
 *  \config       defined(NMOSEK_SLEEPACK_NOTIFICATION_ENABLED)
 ...
 *  \note         Called by Application
 *********************************************************************************************************************/
extern FUNC(boolean, NMOSEK_CODE) NmOsek_GetSendSleepAck(CONST(NetworkHandleType, AUTOMATIC) nmChannelHandle);

/**********************************************************************************************************************
 *  NmOsek_SendSleepAck()
 *********************************************************************************************************************/
/*! \brief        This API triggers NmOsek to directly send a Sleep Ack message in the next NmOsek_MainFunction()
 *  \details      The RING message with the Sleep Acknowledge bit set to 1 will be sent in the context of the next
 *                NmOsek_MainFunction() call if all of the following conditions are met:
 *                - the current Nm state is Repeat Message, Normal Operation or Ready Sleep
 *                - there is no BusOff recovery ongoing
 *                - the Active internal state flag is set
 *                - the network is not requested
 *  \param[in]    nmChannelHandle         Identification of the NM-channel
 *  \return       E_OK      Sending Sleep Ack is going to be triggered in the next NmOsek_MainFunction()
 *  \return       E_NOT_OK  Sending Sleep Ack is not possible
 *  \context      TASK|ISR1|ISR2
 *  \reentrant    FALSE
 *  \synchronous  TRUE
 *  \config       defined(NMOSEK_SLEEPACK_NOTIFICATION_ENABLED)
 ...
 *  \note         Called by Application
 *********************************************************************************************************************/
extern FUNC(Std_ReturnType, NMOSEK_CODE) NmOsek_SendSleepAck(CONST(NetworkHandleType, AUTOMATIC) nmChannelHandle);
# endif
...
# if defined(NMOSEK_SLEEPACK_NOTIFICATION_ENABLED)
/**********************************************************************************************************************
 *  Appl_NmOsek_SendSleepAck()
 *********************************************************************************************************************/
/*! \brief        This function asks the application whether the Sleep Acknowledge Bit shall be set to 1 in the current
 *                RING message or not
 *  \details      NmOsek calls this function when a RING message transmission request will be issued in case it is
 *                allowed to send a RING message with the Sleep Acknowledge Bit set to 1.
 *  \param[in]    nmChannelHandle         Identification of the NM-channel [optional]
 *  \return       TRUE  the Sleep Acknowledge Bit shall be set to 1
 *  \return       FALSE the Sleep Acknowledge Bit shall be set to 0
 *  \context      TASK
 *  \reentrant    FALSE
 *  \synchronous  TRUE
 *  \config       (NMOSEK_SKIPPED_ALIVE_MESSAGE_REQUEST_ENABLED) && (NMOSEK_NM_PASSIVE_MODE_ENABLED == STD_OFF)
 *  \pre          -
 *  \note         Called by NmOsek
 *  \trace        CREQ-119850
 *********************************************************************************************************************/
extern FUNC(boolean, NMOSEK_APPL_CODE) Appl_NmOsek_SendSleepAck(NMOSEK_CHANNEL_APPLTYPE_ONLY);
# endif
