...
NMOSEK_LOCAL_INLINE FUNC(void, NMOSEK_CODE) NmOsek_MainFunction_ActionStart(NMOSEK_CHANNEL_NMTYPE_ONLY)
{
  ...
  NmOsek_SetPrepSendSleepAck(NMOSEK_CHANNEL_IDX, FALSE);
  /* No need to clear Sleep Ack here, is removed below */
#endif
...
# endif
       ...
       NMOSEK_INTERNAL_STATE_BUS_SLEEP_IND_FLAG_MASK) != 0u))
  {
    ...
    {
# if defined(NMOSEK_SLEEPACK_NOTIFICATION_ENABLED)
      NmOsek_SetPrepSendSleepAck(NMOSEK_CHANNEL_IDX, TRUE);

      /* let application decide if SleepAck should be set */ /* ESCAN00086024 */
      NmOsek_UpdateTxMessageData(NMOSEK_CHANNEL_PARA_FIRST NmOsek_GetLogicalSuccessor(NMOSEK_CHANNEL_IDX),
        NMOSEK_RING, (NmOsek_TxMessageDataType)(NMOSEK_SLEEP_IND |
        NMOSEK_CALCULATE_SLEEP_ACK_FROM_BOOLEAN(Appl_NmOsek_SendSleepAck(NMOSEK_CHANNEL_APPLPARA_ONLY))));
# else
...
#if defined(NMOSEK_SLEEPACK_NOTIFICATION_ENABLED)
  NmOsek_SetPrepSendSleepAck(NMOSEK_CHANNEL_IDX, FALSE);
  NmOsek_SetTxMessageData(NMOSEK_MESSAGE_DATA_BYTE_INDEX(NMOSEK_CHANNEL_IDX, NMOSEK_CFG_PDU_SLEEP_POSITION),
                          (NmOsek_TxMessageDataType)
                          (NmOsek_GetTxMessageData(NMOSEK_MESSAGE_DATA_BYTE_INDEX(NMOSEK_CHANNEL_IDX,
                                                   NMOSEK_CFG_PDU_SLEEP_POSITION)) & NMOSEK_SLEEP_ACK_INV));
#endif
...
# if defined(NMOSEK_IMMEDIATE_ALIVE)
      /* - #110 Prepare transmission of immediate wake-up frame/immediate alive message */
      NmOsek_SetInternalState(NMOSEK_CHANNEL_IDX, (NmOsek_InternalStateType)
        (NmOsek_GetInternalState(NMOSEK_CHANNEL_IDX) & NMOSEK_INTERNAL_STATE_PREP_SLEEP_INV_MASK));

      /* - #120 Restart NM immediately */
      /* ESCAN00052322 */
      NmOsek_SetAction(NMOSEK_CHANNEL_IDX, NMOSEK_ACTION_START);

      /* Directly change the Nm State if possible */
      NmOsek_ChangeNmStateIfPossible(NMOSEK_CHANNEL_PARA_FIRST NM_STATE_REPEAT_MESSAGE);
# endif /* defined(NMOSEK_IMMEDIATE_ALIVE) */
...
#if defined(NMOSEK_SLEEPACK_NOTIFICATION_ENABLED)
        NmOsek_SetPrepSendSleepAck(NMOSEK_CHANNEL_IDX, TRUE);
        NmOsek_UpdateTxMessageData(NMOSEK_CHANNEL_PARA_FIRST NmOsek_GetNodeIdOfPbChannelConfig(NMOSEK_CHANNEL_IDX),
                                   NMOSEK_RING, (NmOsek_TxMessageDataType)(NMOSEK_SLEEP_IND | NMOSEK_SLEEP_ACK));
#endif
 ...
 *
 */
FUNC(boolean, NMOSEK_CODE) NmOsek_GetPrepareSendSleepAck(CONST(NetworkHandleType, AUTOMATIC) nmChannelHandle)
{
  /* ----- Local Variables ---------------------------------------------- */
# if (NMOSEK_DEV_ERROR_DETECT == STD_ON)
  VAR(boolean, AUTOMATIC) retVal = FALSE;
...
# endif
    ...
    /* - #30 Return the value of the Prepare Send Sleep Ack flag */
    retVal = (boolean)(NmOsek_IsPrepSendSleepAck(NMOSEK_CHANNEL_IDX));
  }
...
# if (NMOSEK_DEV_ERROR_REPORT == STD_ON)
  if (errorId != NMOSEK_E_NO_ERROR)
  {
    (void)Det_ReportError(NMOSEK_MODULE_ID, NMOSEK_INSTANCE_ID, NMOSEK_SID_GETPREPARESENDSLEEPACK, errorId);
  }
# else
  NMOSEK_DUMMY_STATEMENT(errorId); /* PRQA S 3112, 3199 */ /* MD_MSR_14.2 */ /*lint -e{438} */
# endif
 ...
 *
 */
FUNC(boolean, NMOSEK_CODE) NmOsek_GetSendSleepAck(CONST(NetworkHandleType, AUTOMATIC) nmChannelHandle)
{
  /* ----- Local Variables ---------------------------------------------- */
# if (NMOSEK_DEV_ERROR_DETECT == STD_ON)
  VAR(boolean, AUTOMATIC) retVal = FALSE;
...
# endif
    ...
    /* - #30 Return the value of the Sleep Ack flag from the TX Message Data buffer */
    retVal = (boolean)((NmOsek_GetTxMessageData(NMOSEK_MESSAGE_DATA_BYTE_INDEX(NMOSEK_CHANNEL_IDX, NMOSEK_CFG_PDU_SLEEP_POSITION)) & NMOSEK_OPCODE_SLEEP_MASK) ==
                        NMOSEK_OPCODE_SLEEP_MASK);
  }
...
# if (NMOSEK_DEV_ERROR_REPORT == STD_ON)
  if (errorId != NMOSEK_E_NO_ERROR)
  {
    (void)Det_ReportError(NMOSEK_MODULE_ID, NMOSEK_INSTANCE_ID, NMOSEK_SID_GETSENDSLEEPACK, errorId);
  }
# else
  NMOSEK_DUMMY_STATEMENT(errorId); /* PRQA S 3112, 3199 */ /* MD_MSR_14.2 */ /*lint -e{438} */
# endif
 ...
 *
 */
FUNC(Std_ReturnType, NMOSEK_CODE) NmOsek_SendSleepAck(CONST(NetworkHandleType, AUTOMATIC) nmChannelHandle)
{
  /* ----- Local Variables ---------------------------------------------- */
  VAR(Std_ReturnType, AUTOMATIC) retVal = E_NOT_OK;
  VAR(uint8, AUTOMATIC) errorId = NMOSEK_E_NO_ERROR;
...
# endif
    ...
    /* - #30 If channel state is Ready Sleep, Normal Operation or Repeat Message and the network is not requested,
     *       and the internal state flag active is set and the current state is not BusOff, send the RING message with
     *       Sleep Ack.
     */
    if ((NmOsek_GetDesiredOrActualNmState(NMOSEK_CHANNEL_IDX) >= NM_STATE_READY_SLEEP) &&
# if (NMOSEK_BUS_OFF_NOTIFICATION_ENABLED == STD_ON) || (NMOSEK_FIRST_MESSAGE_SHALL_BE_NM_MESSAGE == STD_ON)
        (NmOsek_GetDesiredOrActualNmState(NMOSEK_CHANNEL_IDX) <= NM_STATE_REPEAT_MESSAGE) && /* ESCAN00084494 */
# else
        /* always true */
# endif
# if (NMOSEK_NM_COM_CONTROL_ENABLED == STD_ON)
        ((NmOsek_GetInternalState(NMOSEK_CHANNEL_IDX) &
         (NMOSEK_INTERNAL_STATE_BUS_SLEEP_IND_FLAG_MASK | NMOSEK_INTERNAL_STATE_ACTIVE_FLAG_MASK)) ==
        (NMOSEK_INTERNAL_STATE_BUS_SLEEP_IND_FLAG_MASK | NMOSEK_INTERNAL_STATE_ACTIVE_FLAG_MASK))
# else
        ((NmOsek_GetInternalState(NMOSEK_CHANNEL_IDX) & NMOSEK_INTERNAL_STATE_BUS_SLEEP_IND_FLAG_MASK) != 0u)
        /* (NmOsek_GetInternalState(NMOSEK_CHANNEL_IDX) & NMOSEK_INTERNAL_STATE_ACTIVE_FLAG_MASK) != 0u:
...
# endif
       )
    {
      ...
      NmOsek_SetInternalState(NMOSEK_CHANNEL_IDX, (NmOsek_InternalStateType)
                              (((NmOsek_GetInternalState(NMOSEK_CHANNEL_IDX) &
                                 NMOSEK_INTERNAL_STATE_IM_SKIPPED_OVER_INV_MASK) &
                                NMOSEK_INTERNAL_STATE_RING_MSG_RECV_INV_MASK) |
                               (NMOSEK_INTERNAL_STATE_PREP_SLEEP_FLAG_MASK |
                                NMOSEK_INTERNAL_STATE_IM_ADDRESSED_FLAG_MASK)));
      NmOsek_SetPrepSendSleepAck(NMOSEK_CHANNEL_IDX, TRUE);
...
# endif
      ...
      if (NmOsek_GetLogicalSuccessor(NMOSEK_CHANNEL_IDX) == NmOsek_GetNodeIdOfPbChannelConfig(NMOSEK_CHANNEL_IDX))
      {
        NmOsek_SetLogicalSuccessor(NMOSEK_CHANNEL_IDX, (NmOsek_LogicalSuccessorType)
                                   (~(NmOsek_LogicalSuccessorType)NmOsek_GetNodeIdOfPbChannelConfig(NMOSEK_CHANNEL_IDX)));
      }
...
# if (NMOSEK_DEV_ERROR_REPORT == STD_ON)
  if (errorId != NMOSEK_E_NO_ERROR)
  {
    (void)Det_ReportError(NMOSEK_MODULE_ID, NMOSEK_INSTANCE_ID, NMOSEK_SID_SENDSLEEPACK, errorId);
  }
# else
  NMOSEK_DUMMY_STATEMENT(errorId); /* PRQA S 3112, 3199 */ /* MD_MSR_14.2 */ /*lint -e{438} */
# endif
